...
internal sealed partial class FinanceDatasetIntakeStore
{
    ...
    private readonly FinanceDatasetQuarantine _quarantine;
    public FinanceDatasetIntakeStore(EodhdFinanceOptions market, FinanceDatasetOptions options)
    {
        _market = market; _options = options; _quarantine = new(options); _quarantine.EnsureDirectory(); Initialize();
    }
    private string ConnectionString => new SqliteConnectionStringBuilder { DataSource = _market.DatabasePath }.ToString();
    private void Initialize()
    {
        using var c = new SqliteConnection(ConnectionString); c.Open(); using var x = c.CreateCommand(); x.CommandText = """
      ...
      """; x.ExecuteNonQuery(); InitializeResearchDatasetStorage(c); InitializeResearchCampaignStorage(c); if (!ColumnExists(c, "dataset_candidates", "cleanup_state")) Exec(c, "ALTER TABLE dataset_candidates ADD COLUMN cleanup_state TEXT NOT NULL DEFAULT 'Retained'");
        ...
        foreach (var item in pending) { var exists = _quarantine.PayloadExists(item.Id, item.File); Exec(c, "UPDATE dataset_candidates SET cleanup_state=$state WHERE candidate_id=$id AND cleanup_state='CleanupPending'", ("$state", exists ? "Retained" : "PayloadDeleted"), ("$id", item.Id)); }
    }

    internal void Discover(ExternalDatasetCandidate candidate, string method)
    {
        using var c = new SqliteConnection(ConnectionString); c.Open(); Exec(c, "INSERT OR IGNORE INTO dataset_candidates(candidate_id,source,source_url,hosting_platform,filename,state,license_class,declared_license,license_evidence_url,license_retrieved_on,provenance,provenance_result,downloader_version,acquisition_method,updated_utc,manifest_json) " +
      "VALUES($id,$source,$url,$host,$file,'Discovered',$license,$declared,$evidence,$date,$provenance,$provenanceResult,'bigbrain-dataset-downloader-v1',$method,$now,$manifest)",
      ...
      ("$provenance", candidate.Provenance), ("$provenanceResult", candidate.Rights.UnderlyingProvenance.ToString()), ("$method", method), ("$now", DateTimeOffset.UtcNow.ToString("O")), ("$manifest", JsonSerializer.Serialize(new { candidate.CanonicalProduct }, Json)));
    }
    ...
    internal async Task<string> DownloadAsync(ExternalDatasetCandidate candidate, CancellationToken token)
    {
        ...
        var expected = candidate.ExpectedBytes ?? _options.MaximumDownloadBytes; _quarantine.EnsureDownloadSpace(expected);
        var target = _quarantine.PrepareDownloadPath(candidate.CandidateId, candidate.OriginalFilename);
        ...
        try
        {
            for (var attempt = 0; ; attempt++)
            {
                requests++; try
                {
                    using var response = await client.GetAsync(candidate.SourceUrl, HttpCompletionOption.ResponseHeadersRead, token); response.EnsureSuccessStatusCode();
                    ...
                    var buffer = new byte[81920]; long total = 0; for (int read; (read = await input.ReadAsync(buffer, token)) > 0;) { total += read; if (total > _options.MaximumDownloadBytes) throw new InvalidDataException("Artifact exceeds configured size limit."); await output.WriteAsync(buffer.AsMemory(0, read), token); }
                    break;
                }
    ...
    private DatasetCatalogItem InspectValidate(ExternalDatasetCandidate candidate, string artifactPath, bool promoteWhenApproved)
    {
        ...
        candidate = WithStoredCanonicalIdentityClaim(candidate);
        ...
        if (promoteWhenApproved && decision.Result == DatasetEvidenceResult.Pass)
        {
            try { _ = CanonicalDatasetRevisionIdentityV2.NormalizeIdentifier(candidate.SourceName); _ = CanonicalDatasetRevisionIdentityV2.NormalizeIdentifier(candidate.CanonicalProduct); }
            catch (InvalidDataException)
            {
                parsed.Limitations.Add("CanonicalSourceOrProductMissingOrInvalid");
                validation = validation with { Limitations = parsed.Limitations.ToImmutableArray() };
                decision = new(DatasetPromotionPolicyV1.Id, DatasetEvidenceResult.Fail, DatasetCandidateState.Rejected, "canonical.identity.missingOrInvalid");
            }
        }
    ...
    internal QuarantineCleanupResult CleanupRejected(DateTimeOffset cutoffUtc)
    {
        ...
        foreach (var item in candidates) { Exec(c, "UPDATE dataset_candidates SET cleanup_state='CleanupPending' WHERE candidate_id=$id AND state='Rejected' AND cleanup_state='Retained'", ("$id", item.Id)); if (_quarantine.DeletePayloadIfPresent(item.Id, item.File)) { deleted++; released += item.Bytes; } Exec(c, "UPDATE dataset_candidates SET cleanup_state='PayloadDeleted' WHERE candidate_id=$id AND state='Rejected' AND cleanup_state='CleanupPending'", ("$id", item.Id)); }
    ...
    private ParsedDataset ParseCsv(string path, ExternalDatasetCandidate candidate)
    {
        ...
        var headerLine = reader.ReadLine() ?? throw new InvalidDataException("CSV is empty."); if (headerLine.Length > 64_000) throw new InvalidDataException("CSV header is too large."); var headers = FinanceDatasetCsvTokenizer.Tokenize(headerLine).Select(x => x.Trim()).ToArray(); var map = headers.Select((h, i) => (h: Normalize(h), i)).ToDictionary(x => x.h, x => x.i, StringComparer.Ordinal);
        var ticker = Column(map, "ticker", "symbol"); var date = Column(map, "date"); var open = Column(map, "open"); var high = Column(map, "high"); var low = Column(map, "low"); var close = Column(map, "close"); var volume = Column(map, "volume"); var hasAdjusted = map.TryGetValue("adjclose", out var adjusted); var hasExDividend = map.TryGetValue("exdividend", out var ex); var hasSplitRatio = map.TryGetValue("splitratio", out var split);
        string? line; long ordinal = 1; while ((line = reader.ReadLine()) is not null)
        {
            ordinal++; if (line.Length > 1_000_000) throw new InvalidDataException("CSV line exceeds safety limit."); var f = FinanceDatasetCsvTokenizer.Tokenize(line); if (f.Count != headers.Length) throw new InvalidDataException($"Malformed CSV field count at row {ordinal}.");
            ...
            var symbol = f[ticker].Trim().ToUpperInvariant(); if (symbol.Length is 0 or > 32 || symbol[0] is '=' or '+' or '-' or '@') { invalid++; continue; }
            var row = new ImportBar(symbol, d, o, h, l, cl, adj, vol, hasExDividend ? f[ex] : null, hasSplitRatio ? f[split] : null, ordinal);
            if (o <= 0 || h <= 0 || l <= 0 || cl <= 0) { nonPositive++; invalid++; continue; }
            if (vol < 0) { invalidVolume++; invalid++; continue; }
            if (h < Math.Max(o, cl) || l > Math.Min(o, cl) || l > h) { inconsistent++; invalid++; continue; }
            if (vol == 0) zeroVolume++; if (lastSourceDate.TryGetValue(symbol, out var previous) && d < previous) outOfOrder++; lastSourceDate[symbol] = d; var key = $"{symbol}|{d:yyyy-MM-dd}"; if (seen.TryGetValue(key, out var old)) { if (old == row with { Row = old.Row }) duplicates++; else conflicts++; continue; }
            seen.Add(key, row); rows.Add(row);
        }
    ...
    private ExternalDatasetCandidate WithStoredCanonicalIdentityClaim(ExternalDatasetCandidate candidate)
    {
        using var connection = new SqliteConnection(ConnectionString); connection.Open();
        using var command = connection.CreateCommand();
        command.CommandText = "SELECT source,manifest_json FROM dataset_candidates WHERE candidate_id=$id";
        command.Parameters.AddWithValue("$id", candidate.CandidateId);
        using var reader = command.ExecuteReader();
        if (!reader.Read()) throw new InvalidDataException("Candidate identity claim is unavailable.");
        using var manifest = JsonDocument.Parse(reader.IsDBNull(1) ? "{}" : reader.GetString(1));
        var product = manifest.RootElement.TryGetProperty("canonicalProduct", out var value) && value.ValueKind == JsonValueKind.String
            ? value.GetString() : null;
        return candidate with { SourceName = reader.GetString(0), CanonicalProduct = product };
    }

    private string Promote(ExternalDatasetCandidate candidate, ParsedDataset parsed)
    {
        var selected = parsed.Rows.Where(x => Watchlist.ContainsKey(x.Symbol)).OrderBy(x => x.Symbol, StringComparer.Ordinal).ThenBy(x => x.Date).ToArray(); if (selected.Length == 0) throw new InvalidOperationException("No safely mapped watchlist rows are available."); var identity = CanonicalDatasetRevisionIdentityV2.Create(candidate.SourceName, candidate.CanonicalProduct, selected.Select(x => new CanonicalDatasetIdentityRow(x.Symbol, x.Date, x.Open, x.High, x.Low, x.Close, x.AdjustedClose, x.Volume))); var hash = identity.Checksum; var revision = identity.RevisionId; var product = identity.Product; using var c = new SqliteConnection(ConnectionString); c.Open(); using var t = c.BeginTransaction(); var now = DateTimeOffset.UtcNow.ToString("O");
        foreach (var row in selected) { var i = Watchlist[row.Symbol]; Exec(c, t, "INSERT OR IGNORE INTO observations VALUES($provider,$product,$policy,$instrument,$symbol,$providerSymbol,$mic,$date,$open,$high,$low,$close,$adjusted,$volume,$acquired,$revision)", ("$provider", identity.Source), ("$product", product), ("$policy", DatasetPromotionPolicyV1.Id), ("$instrument", i.InstrumentId), ("$symbol", row.Symbol), ("$providerSymbol", row.Symbol), ("$mic", i.Mic), ("$date", row.Date.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture)), ("$open", Text(row.Open)), ("$high", Text(row.High)), ("$low", Text(row.Low)), ("$close", Text(row.Close)), ("$adjusted", row.AdjustedClose is null ? "" : Text(row.AdjustedClose.Value)), ("$volume", Convert.ToInt64(row.Volume)), ("$acquired", now), ("$revision", revision)); if (row.ExDividend is not null || row.SplitRatio is not null) Exec(c, t, "INSERT OR IGNORE INTO dataset_corporate_actions VALUES($candidate,$symbol,$date,$dividend,$split,$row)", ("$candidate", candidate.CandidateId), ("$symbol", row.Symbol), ("$date", row.Date.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture)), ("$dividend", row.ExDividend ?? ""), ("$split", row.SplitRatio ?? ""), ("$row", row.Row)); }
    ...
    internal static bool TryRun(string[] args, IConfiguration configuration)
    {
        ...
        var candidate = args[1] switch
        {
            "wiki" => new ExternalDatasetCandidate("wiki-eod-mirror-kmfranz-v1", "NASDAQ-WIKI", "https://github.com/kmfranz/trading_pairs", "GitHub Git LFS", "WIKI_PRICES.csv",
                ...
                "GitHub mirror of Nasdaq/Quandl WIKI/PRICES; artifact identity is validated by schema, coverage and overlap.", DatasetPriceBasis.RawAndAdjusted, DatasetSurvivorshipBias.SurvivorshipUnknown, CanonicalProduct: "PRICES"),
