...
public sealed record FinanceResearchOpportunity(string OpportunityId, string SchedulerVersion, DateOnly ResearchDate,
    ...
    FinanceResearchResourceDecision? ResourceDecision = null);
...
internal sealed partial class EodhdMarketMemory
{
    private static void InitializeResearchSchedulerStorage(SqliteConnection connection)
    {
        using var command = connection.CreateCommand(); command.CommandText = """
          ...
          """; command.ExecuteNonQuery(); AddColumn(connection, "research_schedule_opportunities", "resource_decision_json", "TEXT");
    }
    ...
    {
        using var c = new SqliteConnection(ConnectionString); c.Open(); Execute(c, null, "INSERT OR IGNORE INTO research_schedule_opportunities(opportunity_id,scheduler_version,research_date,due_utc,created_utc,state) VALUES($id,$version,$date,$due,$created,'Pending')", ("$id", id), ("$version", version), ("$date", researchDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture)), ("$due", due.ToString("O", CultureInfo.InvariantCulture)), ("$created", now.ToString("O", CultureInfo.InvariantCulture))); return ReadResearchOpportunity(c, id)!;
    }
    ...
    internal FinanceResearchOpportunity RecordResearchResourceDecision(string id, FinanceResearchResourceDecision decision)
    { using var c = new SqliteConnection(ConnectionString); c.Open(); Execute(c, null, "UPDATE research_schedule_opportunities SET resource_decision_json=$json WHERE opportunity_id=$id", ("$json", JsonSerializer.Serialize(decision)), ("$id", id)); return ReadResearchOpportunity(c, id)!; }
    ...
    internal FinanceResearchDataReadiness ResearchDataReadiness(DateOnly requiredDate)
    {
        ...
        foreach (var instrument in expected)
        {
            ...
            if (!r.Read()) { missing.Add(instrument); continue; }
            var date = DateOnly.Parse(r.GetString(0), CultureInfo.InvariantCulture); if (date != requiredDate) { stale.Add(instrument); continue; }
    ...
    private static FinanceResearchOpportunity? ReadResearchOpportunity(SqliteConnection c, string id) { using var x = c.CreateCommand(); x.CommandText = "SELECT scheduler_version,research_date,due_utc,created_utc,attempted_utc,completed_utc,state,research_run_id,reason,next_eligibility_utc,resource_decision_json FROM research_schedule_opportunities WHERE opportunity_id=$id"; x.Parameters.AddWithValue("$id", id); using var r = x.ExecuteReader(); if (!r.Read()) return null; return new(id, r.GetString(0), DateOnly.Parse(r.GetString(1), CultureInfo.InvariantCulture), DateTimeOffset.Parse(r.GetString(2), CultureInfo.InvariantCulture), DateTimeOffset.Parse(r.GetString(3), CultureInfo.InvariantCulture), r.IsDBNull(4) ? null : DateTimeOffset.Parse(r.GetString(4), CultureInfo.InvariantCulture), r.IsDBNull(5) ? null : DateTimeOffset.Parse(r.GetString(5), CultureInfo.InvariantCulture), Enum.Parse<FinanceResearchOpportunityState>(r.GetString(6)), r.IsDBNull(7) ? null : r.GetString(7), r.IsDBNull(8) ? null : r.GetString(8), r.IsDBNull(9) ? null : DateTimeOffset.Parse(r.GetString(9), CultureInfo.InvariantCulture), r.IsDBNull(10) ? null : JsonSerializer.Deserialize<FinanceResearchResourceDecision>(r.GetString(10))); }
}

internal sealed class FinanceResearchOrchestrator(FinanceResearchSchedulerOptions options, EodhdMarketMemory memory, IFinanceResearchResourceGovernor governor, FinanceResearchOperationsOptions? operations = null)
{
    internal FinanceResearchOpportunity? CheckAndRun(DateTimeOffset nowUtc, bool mayStartTimeSensitiveWork, CancellationToken token) => CheckAndRunAsync(nowUtc, mayStartTimeSensitiveWork, token).GetAwaiter().GetResult();
    internal async Task<FinanceResearchOpportunity?> CheckAndRunAsync(DateTimeOffset nowUtc, bool mayStartTimeSensitiveWork, CancellationToken token)
    {
        options.Validate(); operations ??= new(); operations.Validate(); if (!options.Enabled) return null; if (nowUtc.Offset != TimeSpan.Zero) throw new ArgumentException("Research scheduler requires UTC time."); token.ThrowIfCancellationRequested(); memory.RecordResearchOperationsEvaluation(nowUtc, null); FinanceResearchOpportunity Finish(FinanceResearchOpportunity value) { memory.RecordResearchOperationsEvaluation(nowUtc, value); return value; }
        var due = FinanceResearchSchedule.DueFor(nowUtc, options.ScheduledUtcHour); if (nowUtc < due) return null; var researchDate = FinanceResearchSchedule.ResearchDate(due); memory.SupersedeDeferredResearchOpportunities(researchDate, nowUtc); var id = FinanceResearchSchedule.OpportunityId(options.SchedulerVersion, researchDate); var opportunity = memory.CreateOrReadResearchOpportunity(id, options.SchedulerVersion, researchDate, due, nowUtc);
        if (opportunity.State is FinanceResearchOpportunityState.Completed or FinanceResearchOpportunityState.Failed or FinanceResearchOpportunityState.Skipped) return Finish(opportunity);
        if (opportunity is { State: FinanceResearchOpportunityState.Deferred, NextEligibilityUtc: not null } && opportunity.NextEligibilityUtc > nowUtc) return Finish(opportunity);
        if (!UsMarketCalendar.IsSession(researchDate)) return Finish(memory.UpdateResearchOpportunity(id, FinanceResearchOpportunityState.Skipped, nowUtc, null, "finance.research.scheduler.nonResearchDay", null));
        if (!mayStartTimeSensitiveWork) return Finish(memory.UpdateResearchOpportunity(id, FinanceResearchOpportunityState.Deferred, nowUtc, null, "finance.research.scheduler.recoveryNotReady", nowUtc.AddMinutes(options.CheckIntervalMinutes)));
        if (operations.MaintenancePaused) return Finish(memory.UpdateResearchOpportunity(id, FinanceResearchOpportunityState.Deferred, nowUtc, null, "finance.research.scheduler.maintenancePaused", nowUtc.AddMinutes(options.CheckIntervalMinutes)));
        var readiness = memory.ResearchDataReadiness(researchDate); if (!readiness.IsReady) return Finish(memory.UpdateResearchOpportunity(id, FinanceResearchOpportunityState.Deferred, nowUtc, null, readiness.ReasonCode, nowUtc.AddMinutes(options.CheckIntervalMinutes)));
        var resource = await governor.EvaluateAsync(nowUtc, token); memory.RecordResearchResourceDecision(id, resource); if (resource.Decision != FinanceResearchResourceDecisionKind.Allow) return Finish(memory.UpdateResearchOpportunity(id, FinanceResearchOpportunityState.Deferred, nowUtc, null, resource.ReasonCodes[0], nowUtc.AddMinutes(options.CheckIntervalMinutes)));
        token.ThrowIfCancellationRequested();
        if (!memory.TryClaimResearchOpportunity(id, nowUtc)) { var current = memory.ResearchOpportunity(id)!; return Finish(Reconcile(current, nowUtc)); }
        try { var run = memory.RunAutonomousResearch(id, options.MaximumExperimentsPerRun); return Finish(run.State switch { ResearchExperimentState.Completed => memory.UpdateResearchOpportunity(id, FinanceResearchOpportunityState.Completed, nowUtc, run.RunId, "finance.research.scheduler.completed", null), ResearchExperimentState.Failed => memory.UpdateResearchOpportunity(id, FinanceResearchOpportunityState.Failed, nowUtc, run.RunId, run.FailureReason ?? "finance.research.scheduler.researchFailed", null), _ => memory.UpdateResearchOpportunity(id, FinanceResearchOpportunityState.Started, nowUtc, run.RunId, "finance.research.scheduler.researchRunning", null) }); }
        catch (AutonomousResearchBusyException) { return Finish(memory.UpdateResearchOpportunity(id, FinanceResearchOpportunityState.Deferred, nowUtc, null, "finance.research.scheduler.researchBusy", nowUtc.AddMinutes(options.CheckIntervalMinutes))); }
        catch (CurrentResearchEvidenceUnavailableException exception) { var run = memory.AutonomousResearchRunByKey(id); return Finish(memory.UpdateResearchOpportunity(id, FinanceResearchOpportunityState.Failed, nowUtc, run?.RunId, exception.ReasonCode, null)); }
        catch (OperationCanceledException) when (token.IsCancellationRequested) { return Finish(memory.UpdateResearchOpportunity(id, FinanceResearchOpportunityState.Deferred, nowUtc, null, "finance.research.scheduler.shutdown", nowUtc.AddMinutes(options.CheckIntervalMinutes))); }
...
internal sealed class FinanceResearchSchedulerWorker(FinanceResearchSchedulerOptions options, FinanceResearchOrchestrator orchestrator, SystemRecoveryCoordinator recovery, TimeProvider clock, ILogger<FinanceResearchSchedulerWorker> logger, FinanceResearchOperationsCoordinator? operations = null) : BackgroundService
{
    ...
    private static readonly Action<ILogger, string, Exception?> OperationsTransition = LoggerMessage.Define<string>(LogLevel.Information, new EventId(9501, "FinanceResearchOperationsTransition"), "Finance research operations transitioned to {State}.");
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        FinanceResearchOperationalState? lastOperationsState = null; while (!stoppingToken.IsCancellationRequested) { var result = await orchestrator.CheckAndRunAsync(clock.GetUtcNow(), recovery.MayStartTimeSensitiveWork, stoppingToken); var signature = result is null ? null : $"{result.OpportunityId}:{result.State}:{result.Reason}"; if (result is not null && signature != lastLogged) { Outcome(logger, result.OpportunityId, result.State.ToString(), null); lastLogged = signature; } if (operations is not null) { var state = operations.Status().State; if (state != lastOperationsState && state is FinanceResearchOperationalState.Degraded or FinanceResearchOperationalState.AttentionRequired or FinanceResearchOperationalState.Ready) OperationsTransition(logger, state.ToString(), null); lastOperationsState = state; } if (!await timer.WaitForNextTickAsync(stoppingToken)) break; }
    }
}
