...
export function SystemRecovery({ recovery, error }: { recovery: SystemRecoverySnapshot | null; error: boolean }) {
  ...
  return (
    <div className="system-widget system-recovery">
      <div className="section-actions">
        <StatusBadge status={label[state] ?? state} />
      </div>
      {!recovery && !error && <p aria-live="polite">BigBrain återställer tjänster efter omstart…</p>}
      {error && (
        <p className="notice notice--error" role="alert">
          Recovery-status kunde inte läsas.
        </p>
      )}
      {recovery && (
        <>
          ...
          <div className="metric-grid">
            ...
            <MetricCard
              ...
              value={
                recovery.availableBytes === null ? 'Okänt' : `${(recovery.availableBytes / 1024 ** 3).toFixed(1)} GiB`
              }
            />
            <MetricCard label="Avbrutna jobb" value={String(recovery.interruptedJobs)} />
          </div>
          <details>
            <summary>Komponenter och återställning</summary>
            <div className="module-grid">
              {recovery.components.map(component => (
                <ModuleCard key={component.id} title={component.id} status={component.state}>
          ...
          <p className="last-updated">
            Boot <code>{recovery.bootId.slice(0, 8)}</code> · Finance {recovery.operatingMode}
          </p>
        </>
      )}
    </div>
