...
export interface FinanceResearchSchedulerStatus {currentUtc:string;enabled:boolean;schedulerVersion:string;nextDueUtc:string|null;lastOpportunity:null|{opportunityId:string;researchDate:string;dueAtUtc:string;attemptedAtUtc:string|null;completedAtUtc:string|null;state:string;researchRunId:string|null;reason:string|null;nextEligibilityUtc:string|null};lastResearchRunId:string|null;lastOutcome:string;lastReason:string|null;researchCurrentlyRunning:boolean;operatingMode:'RESEARCH';budgetSek:0;executionAuthority:'NONE'}
