...
export interface FinanceResearchSchedulerStatus {currentUtc:string;enabled:boolean;schedulerVersion:string;nextDueUtc:string|null;lastOpportunity:null|{opportunityId:string;researchDate:string;dueAtUtc:string;attemptedAtUtc:string|null;completedAtUtc:string|null;state:string;researchRunId:string|null;reason:string|null;nextEligibilityUtc:string|null};lastResearchRunId:string|null;lastOutcome:string;lastReason:string|null;researchCurrentlyRunning:boolean;operatingMode:'RESEARCH';budgetSek:0;executionAuthority:'NONE';dataReady:boolean;readinessReason:string;currentInstrumentCount:number;expectedInstrumentCount:number}
...
export interface FinanceResearchOperationsStatus {operationsVersion:string;evaluatedAtUtc:string;state:'disabled'|'maintenance'|'waiting'|'ready'|'running'|'deferred'|'degraded'|'attentionRequired';requiresAttention:boolean;currentActivity:string;schedulerEnabled:boolean;maintenancePaused:boolean;lastSchedulerEvaluationUtc:string|null;lastSuccessfulResearchUtc:string|null;lastOperationalFailureUtc:string|null;consecutiveOperationalFailures:number;lastFailureReason:string|null;lastSuccessfulEvidenceRefreshUtc:string|null;dataReadiness:string;resourceDecision:string|null;activeResearchRunId:string|null;operatingMode:'RESEARCH';budgetSek:0;executionAuthority:'NONE'}
