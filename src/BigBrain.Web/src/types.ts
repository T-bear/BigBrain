...
export interface FinanceAutonomousResearch { generatedAtUtc:string;operatingMode:'RESEARCH';budgetSek:0;engineVersion:string;featureLibraryVersion:string;totalExperiments:number;rejectedCount:number;challengerCount:number;status:string;executionAuthority:'NONE';features:Array<{id:string;version:string;category:string;description:string;sourceDefinitionId:string;complexityCost:number}>;hypotheses:Array<{hypothesisId:string;explanation:string;featureIds:string[];target:string;familyId:string}>;latestRun:null|{runId:string;state:string;experimentCount:number;rejectedCount:number;challengerCount:number;experiments:Array<{experimentId:string;familyId:string;familyAttemptCount:number;verdict:string;rejectionReason:string|null;outOfSampleNetReturn:number|null;costModel:string;featureRevisionId:string;marketRevisionIds:string[];knowledgeCutoffUtc:string;complexity:{score:number};integrity:{state:string;checks:Array<{id:string;state:string;evidence:string}>}}>}}
