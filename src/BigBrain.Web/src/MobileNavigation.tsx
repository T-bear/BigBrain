...
export function MobileNavigation({ dashboards = dashboardRegistry }: { dashboards?: DashboardRegistry }) {
  ...
  return <nav className="mobile-navigation" aria-label="Snabbnavigation">
    {dashboards.getAll().filter(item => primary.has(item.id)).map(item => <button key={item.id} aria-current={activeView === item.id ? 'page' : undefined} onClick={() => setActiveView(item.id)} type="button"><AppIcon name={item.icon} />{item.title}</button>)}
  </nav>
}
