    ...
    async (signal?: AbortSignal) => {
      try {
        ...
        if (remote.configured) apply(remote.theme)
          ...
          await writeTheme(themeRef.current)
          apply(themeRef.current)
        }
        setError('')
      } catch (reason) {
        if (!(reason instanceof DOMException && reason.name === 'AbortError'))
          setError(reason instanceof Error ? reason.message : 'Temainställningen kunde inte hämtas.')
      }
    },
    [apply],
  )
  ...
  const setTheme = useCallback(
    async (next: ThemeId) => {
      ...
      try {
        ...
        apply(previous)
        setError(reason instanceof Error ? reason.message : 'Temainställningen kunde inte sparas.')
      }
    },
    [apply, theme],
  )
