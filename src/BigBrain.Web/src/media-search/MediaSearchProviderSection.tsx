...
export function MediaSearchProviderSection({ provider }: { provider: MediaSearchProviderResult }) {
  ...
  return (
    <section className="media-search-provider" aria-labelledby={headingId}>
      <header>
      ...
      {provider.status === 'online' && provider.results.length === 0 && (
        <p className="media-search-empty">No match in {provider.provider}.</p>
      )}
      {provider.results.length > 0 && (
        <div className="media-search-result-list">
          {provider.results.map(result => (
            <MediaSearchResultCard result={result} key={`${provider.provider}:${result.sourceId}`} />
          ))}
        </div>
      )}
    </section>
