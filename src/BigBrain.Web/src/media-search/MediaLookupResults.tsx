...
export function MediaLookupResults({
  ...
  const ranked = (provider: MediaLookupResponse['providers'][number]) =>
    provider.results
      .map((result, index) => ({
        result,
        index,
        rank:
          result.title.trim().toLocaleLowerCase('sv-SE') === normalizedQuery
            ? 0
            : result.title.toLocaleLowerCase('sv-SE').includes(normalizedQuery)
              ? 1
              : 2,
      }))
  ...
  const providerTitle = (provider: string) =>
    provider === 'Sonarr' ? 'Serier' : provider === 'Radarr' ? 'Filmer' : provider
  return (
    <div className="media-search-results" aria-live="polite">
      <div className="media-search-summary">
        <h3>Resultat för “{response.query}”</h3>
        <span>
          {response.status === 'complete' ? 'Klar' : response.status === 'partial' ? 'Delvis klar' : 'Ej tillgänglig'}
        </span>
      </div>
      ...
      {response.providers.map(provider => {
        ...
        return (
          <section className="media-search-provider" key={provider.provider}>
            <header>
              <h4>{providerTitle(provider.provider)}</h4>
              {provider.status !== 'online' && <span className="provider-status">{provider.status}</span>}
            </header>
            {provider.error && (
            ...
            {results.length > 1 && (
              <button
                className="secondary-button media-results-toggle"
                aria-expanded={expanded}
                type="button"
                onClick={() =>
                  onExpandedProvidersChange(
                    expanded
                      ? expandedProviders.filter(value => value !== provider.provider)
                      : [...expandedProviders, provider.provider],
                  )
                }
              >
                {expanded ? 'Visa färre' : `Visa ${results.length - 1} fler träffar`}
              </button>
            )}
          </section>
        )
