...
test('requires options and preview review before one explicit confirm', async () => {
  ...
  expect(await within(dialog).findByRole('heading', { name: 'Lägg till The Expanse?' })).toBeInTheDocument()
...
test('Escape closes before review and returns focus while raw errors stay hidden', async () => {
  ...
  expect(await within(dialog).findByRole('alert')).toHaveTextContent('Titeln kunde inte läggas till.')
