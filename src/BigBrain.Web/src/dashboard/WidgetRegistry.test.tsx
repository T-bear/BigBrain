...
test('renders registered sections and widgets in declared order', () => {
    ...
    [
      {
        ...
        section: 'widgets',
        order: 20,
        supportedStates: ['online'],
        component: ({ data }) => <span>{data.value} later</span>,
      },
      {
        id: 'first',
        title: 'First',
        ...
        order: 10,
        supportedStates: ['online'],
        component: ({ data }) => <span>{data.value} first</span>,
      },
      {
        id: 'hidden',
        title: 'Hidden',
  ...
  expect([...container.querySelectorAll('.widgets span')].map(element => element.textContent)).toEqual([
    'Widget first',
    'Widget later',
  ])
})
