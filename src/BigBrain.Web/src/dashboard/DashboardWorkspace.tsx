...
function WidgetLibrary({ onClose, view }: { onClose: () => void; view: DashboardViewId }) {
  ...
  return (
    <div
      className="widget-library-backdrop"
      onMouseDown={event => {
    ...
    >
      ...
      >
        <header>
          <div>
            <p className="eyebrow">Anpassa vy</p>
            <h2 id="widget-library-title">Visa widgets</h2>
          </div>
          <BBButton aria-label="Stäng widgetbibliotek" onClick={onClose} type="button" variant="icon">
            ×
          </BBButton>
        </header>
        ...
        <div className="widget-library__list">
          {widgets.map(widget => (
            <label key={widget.id}>
              <input
                checked={!hidden.includes(widget.id)}
                onChange={event => setVisible(view, widget.id, event.target.checked)}
                type="checkbox"
              />
        ...
        <BBButton onClick={onClose} type="button" variant="primary">
          Klar
        </BBButton>
      </div>
    </div>
...
function WidgetFrame({
  ...
  return (
    <section
      ...
      onDrop={event => {
        if (editMode) moveWidgetTo(view, event.dataTransfer.getData('text/widget-id'), definition.id)
      }}
      onDragStart={event => event.dataTransfer.setData('text/widget-id', definition.id)}
    >
      <header className="workspace-module__header">
        <div className="workspace-module__identity">
          <span aria-hidden="true" />
          <div>
            <p>{definition.category}</p>
            <h2 id={`${definition.id}-widget-title`}>{definition.title}</h2>
          </div>
        </div>
        <div className="workspace-module__actions">
          {editMode && (
            <>
              <BBButton
                aria-label={`Flytta ${definition.title} upp`}
                disabled={index === 0}
                onClick={() => moveWidget(view, definition.id, -1)}
                type="button"
                variant="icon"
              >
                ↑
              ...
              <BBButton
                aria-label={`Flytta ${definition.title} ned`}
              ...
              <BBButton
                aria-label={`Dölj ${definition.title}`}
                onClick={() => setVisible(view, definition.id, false)}
                type="button"
                variant="tertiary"
              >
                Dölj
          ...
          <BBButton
            aria-controls={`${definition.id}-widget-content`}
            aria-expanded={!collapsed}
            aria-label={`${collapsed ? 'Expandera' : 'Minimera'} ${definition.title}`}
            onClick={() => toggleCollapsed(view, definition.id)}
            type="button"
            variant="icon"
          >
      ...
      <div className="workspace-module__content" hidden={collapsed} id={`${definition.id}-widget-content`}>
        {definition.render({ expanded: !collapsed })}
      </div>
    </section>
  )
}
...
export function DashboardWorkspace({ dashboards }: { dashboards: DashboardRegistry }) {
  ...
  const ordered = [
    ...viewPreferences.order
      ...
      .filter((widget): widget is WidgetDefinition => Boolean(widget) && widget!.supportedViews.includes(activeView)),
  ...
  useEffect(() => {
    ...
    window.addEventListener('bb:navigation', changed)
    return () => {
      window.removeEventListener('popstate', changed)
      window.removeEventListener('bb:navigation', changed)
    }
  }, [])
  ...
  if (activeView === 'media' && pathname.startsWith('/media/audiobooks')) {
    ...
    return (
      <main className="main bb-page dashboard-workspace dashboard-workspace--media audiobook-route-shell" id="media">
        {audiobooks?.render({ expanded: true })}
      </main>
    )
  }
  ...
  const settings = settingsOpen && (
    <div
      aria-label="Familjeinställningar"
      className="dashboard-settings family-settings"
      ref={settingsRef}
      role="dialog"
    >
      <ThemeControl />
      <button
        ...
        type="button"
      >
        {editMode ? 'Avsluta redigering' : 'Aktivera redigeringsläge'}
      </button>
      <button
        className="secondary-button"
        onClick={() => {
        ...
        type="button"
      >
        Öppna widgetbibliotek
      </button>
    </div>
  )

  if (activeView === 'family' && !editMode)
    return (
      <>
        <FamilyExperience
  ...
  return (
    <main className={`main bb-page dashboard-workspace dashboard-workspace--${activeView}`} id={activeView}>
      <header className="page-header dashboard-workspace__header">
        <div>
          <p className="eyebrow">{dashboard.description}</p>
        ...
        <div className="dashboard-workspace__actions">
          <BBButton
            ...
            aria-label="Dashboardinställningar"
            className="dashboard-settings__trigger"
          ...
          {settingsOpen && (
            <div aria-label="Dashboardinställningar" className="dashboard-settings" ref={settingsRef} role="dialog">
              <ThemeControl />
              <button
                aria-pressed={editMode}
                className="secondary-button"
      ...
      {editMode && (
        <p aria-live="polite" className="notice">
          Redigeringsläge är aktivt. Dra widgets eller använd pilknapparna för att ändra ordning.
        </p>
      )}
      ...
      {ordered.length === 0 && (
        <BBEmptyState detail="Välj vilket innehåll som ska visas i den här vyn." title="Inget innehåll visas">
          <BBButton onClick={() => setLibraryOpen(true)} type="button" variant="primary">
            Visa innehåll
          </BBButton>
        </BBEmptyState>
      )}
