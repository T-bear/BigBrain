...
test('registry exposes complete metadata and filters supported views', () => {
  const registry = new ApplicationWidgetRegistry([
    widget(),
    widget({ id: 'admin-widget', defaultView: 'admin', supportedViews: ['admin'] }),
  ])
...
test('registry rejects duplicate ids and invalid default views', () => {
  ...
  expect(() => new ApplicationWidgetRegistry([widget({ supportedViews: ['media'] })])).toThrow(
    /does not support its default view/,
  )
})
  ...
  const storage = {
    getItem: () =>
      JSON.stringify({
        version: 2,
        activeView: 'home',
        views: { home: { order: ['test-widget'], hidden: [], collapsed: [] } },
      }),
  }
