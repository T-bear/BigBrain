...
export function ShoppingItemRow({
  ...
  const [position, setPosition] = useState({ left: 12, top: 12, visibility: 'hidden' as 'hidden' | 'visible' })
  const trigger = useRef<HTMLButtonElement>(null),
    menu = useRef<HTMLDivElement>(null),
    editInput = useRef<HTMLInputElement>(null)
  useLayoutEffect(() => {
    if (!menuOpen) return
  ...
  useLayoutEffect(() => {
    if (menuOpen && position.visibility === 'visible')
      menu.current?.querySelector<HTMLElement>('button')?.focus({ preventScroll: true })
  }, [menuOpen, position])
  useEffect(() => {
    if (!menuOpen) return
  ...
  return (
    <li className={`shopping-item${item.purchased ? ' shopping-item--purchased' : ''}`}>
      <label className="shopping-item__check">
        <input
          aria-label={`${item.purchased ? 'Återställ' : 'Markera som köpt'} ${item.name}`}
      ...
      {editing ? (
        ...
        <>
          <span className="shopping-item__name">
            ...
            <span className="sr-only">, {item.purchased ? 'köpt' : 'inte köpt'}</span>
          </span>
          <button
            ref={trigger}
            aria-expanded={menuOpen}
            aria-haspopup="menu"
            aria-label={`Åtgärder för ${item.name}`}
            className="shopping-item__menu-trigger"
