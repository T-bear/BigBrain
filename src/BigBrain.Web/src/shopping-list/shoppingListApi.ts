...
async function request<T>(path:string,method='GET',body?:unknown,signal?:AbortSignal):Promise<T>{const response=await fetch(base+path,{method,signal,headers:body?{'Content-Type':'application/json'}:undefined,body:body?JSON.stringify(body):undefined});if(!response.ok){const p=await response.json().catch(()=>null) as {code?:string;detail?:string}|null;throw new ApiError(p?.code??'requestFailed',p?.detail??'Inköpslistan kunde inte uppdateras.')}return response.status===204?undefined as T:response.json()}
