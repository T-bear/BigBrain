...
test('similar product suggests the existing item and requires explicit add anyway', async () => {
  ...
  expect(
    await screen.findByRole('alertdialog', { name: 'Liknande vara finns redan: Lördagsgodis' }),
  ).toBeInTheDocument()
  expect(
  ...
  await waitFor(() =>
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ method: 'POST', body: expect.stringContaining('"addAnyway":true') }),
    ),
  )
})
