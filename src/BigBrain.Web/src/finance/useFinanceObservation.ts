...
export function useFinanceObservation(initialSnapshot?: FinanceObservationSnapshot) {
  ...
  const [snapshot, setSnapshot] = useState<FinanceObservationSnapshot | null>(
    initialSnapshot ?? cachedEntry?.snapshot ?? null,
  )
  ...
  const [selected, setSelected] = useState<string | null>(
    (initialSnapshot ?? cachedEntry?.snapshot)?.watchlist.find(item => item.price !== null)?.instrumentId ?? null,
  )
  ...
  const refresh = useCallback(() => {
    ...
    setRefreshing(true)
    const request = getFinanceObservation(controller.signal)
      .then(value => {
        const fetchedAt = new Date().toISOString()
        setSnapshot(value)
        setSelected(current =>
          value.watchlist.some(item => item.instrumentId === current)
            ? current
            : (value.watchlist.find(item => item.price !== null)?.instrumentId ?? null),
        )
        writeFinanceSnapshotCache(value, fetchedAt)
