...
import { getFinanceBacktest, getFinanceBacktests, getFinanceFeatures, getFinanceObservation } from '../api'
import type { FinanceBacktestCatalog, FinanceBacktestResult, FinanceFeatureSnapshot, FinanceObservationSnapshot } from '../types'
...
export function FinanceObservation({ initialSnapshot, initialFeatures }: { initialSnapshot?: FinanceObservationSnapshot; initialFeatures?: FinanceFeatureSnapshot }) {
  ...
  useEffect(()=>{const controller=new AbortController();getFinanceBacktests(controller.signal).then(value=>{setBacktests(value);setSelectedRun(value.runs[0]?.runId??null)}).catch(()=>setBacktests(null));return()=>controller.abort()},[])
  useEffect(()=>{if(!selectedRun)return;const controller=new AbortController();getFinanceBacktest(selectedRun,controller.signal).then(setBacktestResult).catch(()=>setBacktestResult(null));return()=>controller.abort()},[selectedRun])
  ...
  return <section className="finance-view" aria-labelledby="finance-title">
    ...
    <div className="finance-layout">
      ...
      <section className="finance-panel finance-panel--wide" aria-labelledby="backtests-title"><header><div><h3 id="backtests-title">Backtests / Strategiforskning</h3><p>Offline historisk simulering – ingen order, broker eller handel med riktiga pengar</p></div><span>RESEARCH</span></header>
        {!backtests?.runs.length?<p className="muted">Inga immutable backtestresultat är tillgängliga ännu.</p>:<><div className="finance-backtest-tabs">{backtests.runs.map(run=><button aria-pressed={selectedRun===run.runId} key={run.runId} onClick={()=>setSelectedRun(run.runId)} type="button">{run.strategyId}<small>{run.costModel}</small></button>)}</div>{(()=>{const run=backtests.runs.find(x=>x.runId===selectedRun)??backtests.runs[0];return <><dl className="finance-details"><div><dt>Strategi / version</dt><dd>{run.strategyId} / {run.strategyVersion}</dd></div><div><dt>Period</dt><dd>{run.from} – {run.to}</dd></div><div><dt>Initial / final equity</dt><dd>{run.initialEquity.toFixed(2)} / {run.finalEquity.toFixed(2)} USD</dd></div><div><dt>Gross / net return</dt><dd>{(run.grossReturn*100).toFixed(2)} % / {(run.netReturn*100).toFixed(2)} %</dd></div><div><dt>Benchmark / excess</dt><dd>{run.benchmarkReturn===null?'Benchmark-run':`${(run.benchmarkReturn*100).toFixed(2)} % / ${((run.excessReturn??0)*100).toFixed(2)} %`}</dd></div><div><dt>Max drawdown / trades</dt><dd>{(run.maxDrawdown*100).toFixed(2)} % / {run.trades}</dd></div><div><dt>Kostnadseffekt</dt><dd>{(run.costImpact*100).toFixed(3)} %-enheter</dd></div><div><dt>Reproducerbarhet</dt><dd>{run.runId} / {run.checksum}</dd></div><div><dt>Lineage</dt><dd>{run.marketRevisionIds.length} market-revisioner / {run.featureRevisionId}</dd></div><div><dt>Modeller</dt><dd>{run.simulationModel} / {run.costModel} / {run.sizingPolicy}</dd></div></dl><BacktestCurve result={backtestResult}/><p className="finance-limitations">{run.limitations.join(' ')}</p></>})()}</>}
      </section>
