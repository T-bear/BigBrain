...
export function FinanceObservation({ initialSnapshot }: { initialSnapshot?: FinanceObservationSnapshot }) {
  ...
  return <section className="finance-view" aria-labelledby="finance-title">
    ...
    <div className="finance-layout">
      ...
      {snapshot.retention && <section className="finance-panel" aria-labelledby="retention-title"><header><div><h3 id="retention-title">Retention</h3><p>EODHD-datas livscykel</p></div></header><dl className="finance-details"><div><dt>Status</dt><dd>{label(snapshot.retention.state)}</dd></div><div><dt>Raderingsfrist</dt><dd>{formatTime(snapshot.retention.deletionDeadlineUtc)}</dd></div><div><dt>Omfattning</dt><dd>{snapshot.retention.coveredObservationCount} observationer / {snapshot.retention.coveredRevisionCount} revisioner / {snapshot.retention.coveredPayloadCount} payloads</dd></div>{snapshot.retention.lastReceiptId && <div><dt>Kvitto</dt><dd>{snapshot.retention.lastReceiptId}</dd></div>}</dl></section>}
    </div>
  </section>
}
