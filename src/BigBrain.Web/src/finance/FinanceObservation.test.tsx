...
describe('BB-130C E1 robustness selected-result identity', () => {
  ...
  test('obsolete rejection cannot erase B or restore A after successful B', async () => {
    ...
    await act(async () => { ui.b.resolve(result('B')) })
    ui.coherent('B')
    await act(async () => { ui.a.reject(new DOMException('synthetic obsolete abort', 'AbortError')) })
    ui.coherent('B')
  })
  ...
  test.each(['evaluationId', 'checksum'] as const)('rejects a response with mismatched %s', async field => {
    ...
    await act(async () => { ui.a.resolve(result('A')) })
    ui.select('B')
    await act(async () => { ui.b.resolve({ ...result('B'), [field]: result('A')[field] }) })
    expect(ui.panel.getByText('evaluation-B / checksum-B')).toBeVisible()
    expect(ui.panel.queryByText('parameters-A')).not.toBeInTheDocument()
  ...
  test('matching initial evidence is restored only for its own selection; close keeps the selected identity', async () => {
    ...
    await act(async () => { setResearchOpen(false); b.resolve(result('B')) })
    await act(async () => { setResearchOpen(true) })
