...
const snapshot: FinanceObservationSnapshot = {
  ...
  provider: {
    ...
    entitlementGate: 'policy',
    reason: 'read-only',
  },
  latestMarketDataUpdateUtc: '2026-09-01T00:00:00Z',
  dataKind: 'real',
  watchlist: [
    {
  ...
  historicalMemory: {
    ...
    lastAcquiredAtUtc: '2026-09-02T00:00:00Z',
    gapCount: 0,
    correctionCount: 0,
    persistence: 'durable',
    provider: 'EODHD',
...
describe('Finance last-known-good cache', () => {
  test('ignores malformed and incompatible cache entries', () => {
    ...
    localStorage.setItem(
      FINANCE_SNAPSHOT_CACHE_KEY,
      JSON.stringify({ version: 999, fetchedAtUtc: '2026-09-02T18:00:00Z', sections: ['observation'], snapshot }),
    )
    expect(readFinanceSnapshotCache()).toBeNull()
    localStorage.setItem(
      FINANCE_SNAPSHOT_CACHE_KEY,
      JSON.stringify({
        version: 1,
        fetchedAtUtc: '2026-09-02T18:00:00Z',
        sections: ['observation'],
        snapshot: { ...snapshot, watchlist: [{ ...snapshot.watchlist[0], price: 'not-a-number' }] },
      }),
    )
    expect(readFinanceSnapshotCache()).toBeNull()
  })
  ...
  test('persists only an explicit display-safe projection and never unexpected credential fields', () => {
    const unsafe = {
      ...
      provider: { ...snapshot.provider, authorizationToken: 'must-not-persist' },
    } as FinanceObservationSnapshot
  ...
  test('rejects an unbounded watchlist instead of persisting an arbitrary response', () => {
    const oversized = {
      ...snapshot,
      watchlist: Array.from({ length: 17 }, (_, index) => ({
