...
export function readFinanceSnapshotCache(
  storage: Pick<Storage, 'getItem'> = window.localStorage,
): FinanceSnapshotCacheEntry | null {
  try {
    ...
    if (
      entry.version !== FINANCE_SNAPSHOT_CACHE_VERSION ||
      !validDate(entry.fetchedAtUtc) ||
      entry.sections?.length !== 1 ||
      entry.sections[0] !== 'observation' ||
    ...
    return {
      ...
      sections: ['observation'],
      snapshot: displaySafeSnapshot(entry.snapshot),
    }
  } catch {
    return null
  }
...
export function writeFinanceSnapshotCache(
  snapshot: FinanceObservationSnapshot,
  fetchedAtUtc = new Date().toISOString(),
  storage: Pick<Storage, 'setItem'> = window.localStorage,
) {
  if (!isSnapshot(snapshot) || !validDate(fetchedAtUtc)) return false
  const entry: FinanceSnapshotCacheEntry = {
    version: FINANCE_SNAPSHOT_CACHE_VERSION,
    fetchedAtUtc,
    sections: ['observation'],
    snapshot: displaySafeSnapshot(snapshot),
  }
  const serialized = JSON.stringify(entry)
  if (serialized.length > MAX_CACHE_BYTES) return false
  try {
    storage.setItem(FINANCE_SNAPSHOT_CACHE_KEY, serialized)
    return true
  } catch {
    return false
  }
  ...
  return {
    ...
    watchlist: snapshot.watchlist.map(item => ({
      ...
      history: item.history.map(point => ({
        ...
        value: point.value,
        beginsAfterGap: point.beginsAfterGap,
      })),
    })),
    historicalMemory: {
...
function isSnapshot(value: unknown): value is FinanceObservationSnapshot {
  ...
  if (
    !validDate(item.generatedAtUtc) ||
    !item.safety ||
    item.safety.mode !== 'research' ||
    typeof item.safety.liveTradingEnabled !== 'boolean' ||
    typeof item.safety.paperTradingEnabled !== 'boolean' ||
    typeof item.safety.brokerConnected !== 'boolean' ||
    typeof item.safety.ingestionAllowed !== 'boolean' ||
    typeof item.safety.realProviderStorageAllowed !== 'boolean' ||
    !item.provider ||
    !validString(item.provider.state) ||
    !validString(item.provider.displayName) ||
    !validString(item.provider.entitlement) ||
    !validString(item.provider.entitlementGate) ||
    !validString(item.provider.reason) ||
    (item.provider.evidenceClass !== undefined && !validString(item.provider.evidenceClass)) ||
    !validNullableDate(item.latestMarketDataUpdateUtc) ||
    !['none', 'syntheticFixture', 'real'].includes(item.dataKind) ||
    !item.historicalMemory ||
    !Array.isArray(item.watchlist) ||
    item.watchlist.length > MAX_INSTRUMENTS
  )
    return false
  if (
    !validNumber(item.historicalMemory.observationCount) ||
    !validNullableString(item.historicalMemory.activeRevisionId) ||
    !validNullableString(item.historicalMemory.parentRevisionId) ||
    !validNullableDate(item.historicalMemory.coverageFrom) ||
    !validNullableDate(item.historicalMemory.coverageTo) ||
    !validNullableDate(item.historicalMemory.lastAcquiredAtUtc) ||
    !validNumber(item.historicalMemory.gapCount) ||
    !validNumber(item.historicalMemory.correctionCount) ||
    !validString(item.historicalMemory.persistence) ||
    !validString(item.historicalMemory.provider) ||
    !validString(item.historicalMemory.product) ||
    !validString(item.historicalMemory.policy) ||
    !validString(item.historicalMemory.provenance)
  )
    return false
  if (
    item.retention &&
    (!validString(item.retention.state) ||
      !validNullableDate(item.retention.entitlementEndsAtUtc) ||
      !validNullableDate(item.retention.deletionDeadlineUtc) ||
      !validNumber(item.retention.coveredObservationCount) ||
      !validNumber(item.retention.coveredRevisionCount) ||
      !validNumber(item.retention.coveredPayloadCount) ||
      !validString(item.retention.deletionScope) ||
      !validNullableString(item.retention.lastReceiptId))
  )
    return false
  return item.watchlist.every(
    instrument =>
      validString(instrument.instrumentId) &&
      validString(instrument.symbol) &&
      validString(instrument.displayName) &&
      validNullableNumber(instrument.price) &&
      validNullableString(instrument.currency) &&
      validNullableNumber(instrument.dailyChangePercent) &&
      validNullableDate(instrument.observedAtUtc) &&
      validString(instrument.freshness) &&
      validString(instrument.session) &&
      validString(instrument.quality) &&
      validString(instrument.dataKind) &&
      Array.isArray(instrument.history) &&
      instrument.history.length <= MAX_HISTORY_POINTS &&
      instrument.history.every(
        point =>
          validDate(point.observedAtUtc) &&
          validNullableNumber(point.value) &&
          typeof point.beginsAfterGap === 'boolean',
      ),
  )
}
