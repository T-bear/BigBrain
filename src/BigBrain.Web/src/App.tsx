...
function ViewRequests({
  ...
  onModules: (value: ModuleDefinition[]) => void
  onModuleError: (value: boolean) => void
  onDocker: (value: DockerInventory) => void
  onDockerError: (value: boolean) => void
  onRecovery: (value: SystemRecoverySnapshot) => void
  onRecoveryError: (value: boolean) => void
  onSystem: (value: SystemOverview) => void
  onSystemError: (value: boolean) => void
}) {
  ...
  useEffect(() => {
    ...
    void getModules(controller.signal)
      .then(value => {
      ...
      .catch(() => {
        if (!controller.signal.aborted) onModuleError(true)
      })
  ...
  useEffect(() => {
    ...
    const controller = new AbortController()
    void getSystemRecovery(controller.signal)
      .then(value => {
        if (!controller.signal.aborted) {
          onRecovery(value)
          onRecoveryError(false)
        }
      })
      .catch(() => {
        if (!controller.signal.aborted) onRecoveryError(true)
      })
    return () => controller.abort()
  ...
  useEffect(() => {
    ...
    const controller = new AbortController()
    void getDockerContainers(controller.signal)
      .then(value => {
        if (!controller.signal.aborted) {
          onDocker(value)
          onDockerError(false)
      ...
      .catch(() => {
        if (!controller.signal.aborted) onDockerError(true)
      })
    return () => controller.abort()
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        ...
        if (!controller.signal.aborted) onSystemError(true)
      } finally {
        requestActive = false
      }
    }
...
function AppContent() {
  ...
  const registry = useMemo(
    ...
    [docker, dockerError, moduleError, modules, recovery, recoveryError, system, systemError],
  )
  return (
    <WidgetProvider registry={registry}>
      <ViewRequests
        onModules={setModules}
        onModuleError={setModuleError}
        onDocker={setDocker}
        onDockerError={setDockerError}
