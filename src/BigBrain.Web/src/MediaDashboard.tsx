...
export function MediaDashboard({
  ...
  return (
    <section
      id={administrationOnly ? 'media-administration' : 'media'}
      aria-label={administrationOnly ? 'Mediaadministration' : undefined}
      aria-labelledby={administrationOnly ? undefined : 'media-heading'}
      className="media-section"
    >
      {!administrationOnly && (
        <div className="section-heading">
          <div>
            <p className="eyebrow">Film och serier</p>
            <h2 id="media-heading">Hitta något att titta på</h2>
          </div>
        </div>
      )}
      ...
      {error && (
        <p role="alert" className="notice notice--error">
          Film och serier kunde inte laddas.{overview ? ' Senaste tillgängliga uppdatering visas.' : ''}
        </p>
      )}
      {overview && !administrationOnly && (
        <>
          <div id="search" data-dashboard-module="media-search">
            <MediaSearch />
          </div>
          <SmartShuffle />
          <DownloadControl />
          ...
          >
            <div id="queue">
              <p className="media-flow-intro">
                Följ film och serier genom sökning, nedladdning och bearbetning tills de finns i biblioteket. En titel
                kan därför synas här samtidigt som själva nedladdningen visas i Nedladdningskö.
              </p>
              <MediaJobs showHeading={false} />
            </div>
          </CollapsibleModule>
        </>
      )}
      <details
        ...
        id="administration"
        open={administrationExpanded}
        onToggle={event => setAdministrationExpanded(event.currentTarget.open)}
      >
        <summary>
          <span>
            <strong>Administration</strong>
            <small>Systemstatus, tjänster och diagnostik</small>
          </span>
        </summary>
        <div className="administration__content">
          <div className="administration__heading">
            <div>
              <p className="eyebrow">Teknisk översikt</p>
          ...
          {overview && (
            <DashboardSections
              ...
              registry={mediaWidgetRegistry}
              sectionIds={primarySections}
            />
          )}
          {children}
          {overview && (
            <DashboardSections
              ...
              state={overview.status}
              registry={mediaWidgetRegistry}
              sectionIds={secondarySections}
            />
          )}
          {overview && (
            <p className="last-updated">
              Uppdaterad{' '}
              <time dateTime={overview.collectedAtUtc}>{new Date(overview.collectedAtUtc).toLocaleTimeString()}</time>
            </p>
          )}
        </div>
      </details>
