...
describe('CalendarWidget', () => {
  beforeEach(() => {
    ...
    vi.stubGlobal(
      ...
      vi.fn((input: RequestInfo | URL) => {
        ...
        if (url.includes('/week')) return ok({ from: '2026-08-03', to: '2026-08-09', events: [event] })
        if (url.includes('/month')) return ok({ year: 2026, month: 8, events: [event] })
        if (url.endsWith('/imports')) return ok([])
        return ok({ files: [] })
      }),
    )
  })
  ...
  it('shows safe empty and error states', async () => {
    ...
    expect(await screen.findByText('Inga schemaposter denna vecka.')).toBeInTheDocument()
    unmount()
    vi.mocked(fetch).mockImplementationOnce(() =>
      Promise.resolve(
        new Response(JSON.stringify({ code: 'calendarImportInvalidStructure', detail: 'private detail ignored' }), {
          status: 400,
    ...
    render(<CalendarWidget />)
    expect(await screen.findByRole('alert')).toHaveTextContent('Filen verkar inte vara ett Heroma-schema.')
  })

  it('opens the month, renders time directly, navigates and restores focus on Escape', async () => {
    render(<CalendarWidget />)
    const open = await screen.findByRole('button', { name: 'Öppna kalender' })
    ...
    await waitFor(() => expect(vi.mocked(fetch).mock.calls.some(([url]) => String(url).includes('month=9'))).toBe(true))
    fireEvent.keyDown(document, { key: 'Escape' })
    expect(screen.queryByRole('dialog', { name: /kalender/i })).not.toBeInTheDocument()
  ...
  it('supports multiple files and displays per-file preview actions', async () => {
    vi.mocked(fetch).mockImplementation((input: RequestInfo | URL, init?: RequestInit) => {
          ...
          files: [
            {
              ...
              preview: {
                ...
                year: 2026,
                month: 8,
                counts: { total: 5, day: 1, evening: 1, education: 1, collaboration: 1, vacation: 1, other: 0 },
                skippedRows: 0,
                warningCount: 0,
                monthExists: true,
                exactDuplicate: false,
                existingEventCount: 4,
                conflictCount: 0,
      ...
      if (init?.method === 'POST')
        return ok({ status: 'completed', importedEvents: 5, skippedDuplicates: 0, conflicts: 0 })
      return ok([])
    })
    render(<CalendarWidget />)
    fireEvent.click(await screen.findByRole('button', { name: 'Öppna kalender' }))
    fireEvent.click(await screen.findByRole('button', { name: 'Importera Heroma-schema' }))
    const input = screen.getByLabelText(/Välj en eller flera/)
    const files = [new File(['a'], 'synthetic-a.xlsx'), new File(['b'], 'synthetic-b.xlsx')]
    fireEvent.change(input, { target: { files } })
    expect((input as HTMLInputElement).multiple).toBe(true)
    fireEvent.click(screen.getByRole('button', { name: 'Förhandsgranska' }))
    expect(await screen.findByText('synthetic-a.xlsx')).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Ersätt månad/ })).toBeEnabled()
    expect(screen.getByRole('button', { name: 'Slå ihop' })).toBeEnabled()
  ...
  it('keeps all August days and long import history in one ordered content flow', async () => {
    ...
    render(<CalendarWidget />)
    fireEvent.click(await screen.findByRole('button', { name: 'Öppna kalender' }))
    await screen.findByRole('dialog', { name: /augusti 2026/i })
    expect(await screen.findByText(longName)).toBeInTheDocument()
    const flow = screen.getByTestId('calendar-content-flow'),
      month = screen.getByTestId('calendar-month-list'),
      history = screen.getByTestId('calendar-import-history')
    expect(month.children).toHaveLength(31)
    expect(month.parentElement).toBe(flow)
    expect(history.parentElement).toBe(flow)
