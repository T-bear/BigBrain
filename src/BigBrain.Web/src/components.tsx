...
export function StatusBadge({ status, compact = false }: { status: string; compact?: boolean }) {
  ...
  return (
    <span className={`bb-badge status-badge status-badge--${normalized} ${compact ? 'status-badge--compact' : ''}`}>
      {status}
    </span>
  )
}
...
export const BBButton = forwardRef<
  HTMLButtonElement,
  PropsWithChildren<ButtonHTMLAttributes<HTMLButtonElement> & { variant?: BBButtonVariant; busy?: boolean }>
>(function BBButton({ variant = 'secondary', className = '', busy = false, children, disabled, ...props }, ref) {
  ...
  return (
    <button
      ...
      className={`bb-button bb-button--${variant} ${className}`.trim()}
      disabled={disabled || busy}
      ref={ref}
    >
      <span className={busy ? 'bb-button__content bb-button__content--busy' : 'bb-button__content'}>{children}</span>
      {busy && <BBLoadingIndicator label={busyLabel} compact />}
    </button>
  )
})
...
export function BBLoadingIndicator({ label = 'Laddar', compact = false }: { label?: string; compact?: boolean }) {
  return (
    <span
      aria-live="polite"
      className={`bb-loading-indicator ${compact ? 'bb-loading-indicator--compact' : ''}`}
      role="status"
    >
      <span aria-hidden="true" className="bb-loading-indicator__dots">
        <i />
        <i />
        <i />
      </span>
...
export function BBMediaArtwork({ src, alt, className = '', ...props }: ImgHTMLAttributes<HTMLImageElement>) {
  ...
  if (failed)
    return (
      <div aria-label={alt || 'Omslag saknas'} className={`bb-media-placeholder ${className}`.trim()} role="img">
        <span aria-hidden="true" className="bb-media-placeholder__mark">
          B
        </span>
      </div>
    )
...
export function BBSelect({
  ...
  return (
    <select className={`bb-select ${className}`.trim()} {...props}>
      {children}
    </select>
  )
}

export function BBSurface({
  as = 'section',
  className = '',
  children,
  ...props
}: PropsWithChildren<HTMLAttributes<HTMLElement> & { as?: 'section' | 'article' | 'div' }>) {
  const Component = as
  return (
    <Component className={`bb-surface ${className}`.trim()} {...props}>
      {children}
    </Component>
  )
}
...
export function BBEmptyState({ title, detail, children }: PropsWithChildren<{ title: string; detail?: string }>) {
  return (
    <div className="bb-empty-state" role="status">
      <strong>{title}</strong>
