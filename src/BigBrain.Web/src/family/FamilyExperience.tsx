...
export function FamilyExperience({
  ...
  return (
    <main className="main bb-page family-experience" id="family">
      <header className="family-header">
        ...
        <button
          ...
          className="family-settings-trigger"
      ...
      <div className="family-context" role="status">
        <span aria-hidden="true">◇</span>
        <div>
          <strong>Veckan tillsammans</strong>
          <small>Mat, inköp och kalender på samma plats</small>
        </div>
      </div>
      <div className="family-flow">
        {widgets.map(widget => {
          ...
          return (
            <section
              aria-labelledby={`family-${widget.id}-title`}
              className={`family-section family-section--${widget.id}`}
              data-family-section={widget.id}
              data-widget-id={widget.id}
              key={widget.id}
            >
              <header className="family-section__heading">
                <div>
                  <p>{copy.kicker}</p>
                  <h2 id={`family-${widget.id}-title`}>{copy.title}</h2>
                </div>
