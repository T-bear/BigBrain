...
function Candidate({ candidate, onView }: { candidate: AudiobookAcquisitionCandidate; onView: (candidate: AudiobookAcquisitionCandidate) => void }) {
  ...
  return <BBSurface className="audiobook-candidate"><BBMediaArtwork alt={`Omslag till ${candidate.title}`} loading="lazy" src={candidate.coverUrl ?? undefined} /><div className="audiobook__copy"><h3>{candidate.title}</h3>{candidate.author && <p>{candidate.author}</p>}<small>{details}</small><span className="audiobook-confidence">{candidate.languageConfidence === 'verified' ? 'Verifierat språk' : candidate.languageConfidence === 'probable' ? 'Troligt språk' : 'Språk okänt'}</span><BBButton onClick={() => onView(candidate)} variant="secondary">Välj utgåva</BBButton></div></BBSurface>
}

function readHiddenHistory() {
  try {
    const value = JSON.parse(window.localStorage.getItem(hiddenHistoryKey) ?? '[]')
...
function Activity({ jobs, total, onMore }: { jobs: AudiobookAcquisitionJob[]; total: number; onMore: () => void }) {
  ...
  const historyTotal = Math.max(jobs.filter(job => ['completed', 'cancelled'].includes(job.status)).length, total - active.length - attention.length)
  const hideHistory = () => {
  ...
  return <section aria-labelledby="audiobook-downloads-heading" className="audiobook-activity"><h2 id="audiobook-downloads-heading">Hämtningar</h2>{active.length > 0 && <section aria-labelledby="active-downloads-heading"><h3 id="active-downloads-heading">Aktiva</h3>{active.map(job => <div className="audiobook-job" key={job.id}><strong>{job.candidate.title}</strong><span>{statusLabel(job.status)}</span></div>)}</section>}{attention.length > 0 && <BBSurface><h3>Kräver åtgärd</h3>{attention.map(job => <div className="audiobook-job" key={job.id}><strong>{job.candidate.title}</strong><span>{statusLabel(job.status)}</span>{job.message && <small>{job.message}</small>}</div>)}</BBSurface>}{historyTotal > 0 && <details className="audiobook-history"><summary>Historik ({historyTotal})</summary>{history.map(job => <div className="audiobook-job" key={job.id}><strong>{job.candidate.title}</strong><span>{statusLabel(job.status)}</span></div>)}{history.length === 0 && <p>Visad historik är dold på den här enheten.</p>}<div className="bb-action-group">{total > jobs.length && <BBButton onClick={onMore} variant="tertiary">Visa fler</BBButton>}{history.length > 0 && <BBButton onClick={hideHistory} variant="tertiary">Dölj visad historik</BBButton>}</div><small>Endast presentationen rensas. BigBrains audit- och jobbdata bevaras.</small></details>}</section>
}
...
function Discovery({ busy, discovery, language, metadata, notice, provider, query, onLanguage, onQuery, onSearch, onView }: { busy: boolean; discovery: AudiobookAcquisitionCandidate[] | null; language: string; metadata: AudiobookMetadataResolution | null; notice: string | null; provider: AudiobookAcquisitionProviderStatus | null; query: string; onLanguage: (value: string) => void; onQuery: (value: string) => void; onSearch: (event: FormEvent) => void; onView: (candidate: AudiobookAcquisitionCandidate) => void }) {
  return <section aria-labelledby="new-audiobook-heading" className="audiobook-discovery"><div className="audiobook-section-title"><div><p className="eyebrow">Lägg till</p><h2 id="new-audiobook-heading">Hitta en ny ljudbok</h2></div></div><form className="audiobook-search" onSubmit={onSearch}><label htmlFor="audiobook-query">Sök utanför ditt bibliotek</label><div><BBInput id="audiobook-query" minLength={2} maxLength={120} onChange={event => onQuery(event.target.value)} placeholder="Titel, författare, serie eller ISBN" value={query} /><BBSelect aria-label="Föredraget språk" onChange={event => onLanguage(event.target.value)} value={language}><option value="sv">Svenska</option><option value="en">Engelska</option><option value="all">Alla språk</option></BBSelect><BBButton busy={busy} type="submit" variant="secondary">Sök nya ljudböcker</BBButton></div></form>{notice && <p aria-live="polite" className="audiobook-notice">{notice}</p>}{metadata?.works.length ? <section aria-labelledby="audiobook-metadata-heading"><h3 id="audiobook-metadata-heading">Bokträff</h3><BBSurface className="audiobook-metadata">{metadata.works.slice(0, 3).map(work => <article key={work.workId}><div><strong>{work.canonicalTitle}</strong>{work.authors.length > 0 && <p>{work.authors.join(', ')}</p>}{work.series && <small>Serie: {work.series}</small>}</div></article>)}</BBSurface></section> : null}{discovery?.length ? <><div className="audiobook-section-title"><h3>Kan läggas till</h3><span>{discovery.length} utgåvor</span></div><div className="audiobook-candidates">{discovery.map(item => <Candidate candidate={item} key={`${item.source}:${item.editionId}`} onView={onView} />)}</div></> : null}{provider && !provider.canRequest && <BBSurface className="audiobook-provider-note"><strong>{provider.state === 'configuredUnavailable' ? 'Det går inte att lägga till just nu.' : 'Automatisk hämtning är inte konfigurerad ännu.'}</strong></BBSurface>}</section>
}
...
export function Audiobooks() {
  ...
  const navigate = (path: string, next: Route, item?: AudiobookItem) => { window.history.replaceState({ ...window.history.state, bbAudiobookOrigin: true, scrollY: window.scrollY }, ''); window.history.pushState({ bbAudiobook: true }, '', path); setDirection('forward'); setRoute(next); if (item) setSelected(item); notifyRoute() }
  ...
  if (route.kind === 'overview') return <section aria-labelledby="audiobooks-heading" className="audiobooks-section audiobook-overview"><div className="audiobook-overview__heading"><div><p className="eyebrow">Lyssna</p><h2 id="audiobooks-heading">Ljudböcker</h2></div><BBButton aria-label="Öppna ljudboksbiblioteket" className="audiobook-collection-link" onClick={() => navigate(collectionPath, { kind: 'collection' })} variant="tertiary">Bibliotek <span aria-hidden="true">›</span></BBButton></div>{!data && <BBLoadingIndicator label="Laddar ljudböcker" />}{data?.state === 'notConfigured' && <BBEmptyState title="Audiobookshelf väntar på konfigurering" />}{data?.state === 'configuredUnavailable' && <BBEmptyState title="Ljudböcker är tillfälligt otillgängliga" detail={data.message ?? undefined} />}{data?.continueListening ? <BBSurface className="audiobook-continue-strip"><h3>Fortsätt lyssna</h3><CompactBook item={data.continueListening} onOpen={openDetail} /></BBSurface> : data?.state === 'configuredHealthy' && <p className="audiobook-overview__quiet">Fortsätt lyssna är inte tillgängligt för den anslutna ljudboksprofilen.</p>}</section>

  if (route.kind === 'detail') return <section className={`audiobooks-section audiobook-route-view audiobook-route-view--${direction}`}><BBButton className="audiobook-route-back" onClick={() => goBack(collectionPath, { kind: 'collection' })} variant="tertiary">‹ Ljudböcker</BBButton>{detailBusy && <BBLoadingIndicator label="Öppnar ljudbok" />}{selected && <article className="audiobook-detail-page"><BBMediaArtwork alt={`Omslag till ${selected.title}`} loading="eager" src={selected.coverUrl ?? undefined} /><div><p className="eyebrow">Ljudbok</p><h1 ref={headingRef} tabIndex={-1}>{selected.title}</h1>{selected.author && <p>{selected.author}</p>}{selected.series && <p>Serie: {selected.series}</p>}{selected.narrator && <p>Uppläsare: {selected.narrator}</p>}<p>{selected.languageLabel}{selected.publishedYear ? ` · ${selected.publishedYear}` : ''}</p>{selected.progressPercent !== null && <progress aria-label={`Lyssnat ${Math.round(selected.progressPercent)} procent`} max="100" value={selected.progressPercent} />}{selected.description && <p>{selected.description}</p>}{selected.playbackUrl && <a className="bb-button bb-button--primary" href={selected.playbackUrl} rel="noreferrer">Öppna tillfälligt i Audiobookshelf</a>}</div></article>}{!selected && !detailBusy && notice && <BBEmptyState title={notice} />}</section>

  return <section aria-labelledby="all-audiobooks-heading" className={`audiobooks-section audiobook-route-view audiobook-route-view--${direction}`}><header className="audiobook-route-header"><BBButton className="audiobook-route-back" onClick={() => goBack('/', { kind: 'overview' })} variant="tertiary">‹ Media</BBButton><div><h1 id="all-audiobooks-heading" ref={headingRef} tabIndex={-1}>Ljudböcker</h1><small>{libraryTotal} i biblioteket</small></div></header>{data?.continueListening && <BBSurface className="audiobook-continue-strip"><h2>Fortsätt lyssna</h2><CompactBook item={data.continueListening} onOpen={openDetail} /></BBSurface>}<Discovery busy={busy} discovery={discovery} language={language} metadata={metadata} notice={notice} provider={provider} query={query} onLanguage={setLanguage} onQuery={setQuery} onSearch={search} onView={setSelectedCandidate} />{localResults?.length ? <section aria-labelledby="local-search-heading"><h2 id="local-search-heading">Finns i ditt bibliotek</h2><div className="audiobook-grid">{localResults.map(item => <Book item={item} key={item.id} onOpen={openDetail} />)}</div></section> : null}<Activity jobs={jobs} onMore={moreJobs} total={jobTotal} /><section aria-labelledby="owned-audiobooks-heading" className="audiobook-owned-library"><div className="audiobook-section-title"><div><p className="eyebrow">Din samling</p><h2 id="owned-audiobooks-heading">Ditt bibliotek</h2></div><small>{libraryTotal} ljudböcker</small></div><form className="audiobook-library-tools" onSubmit={findLibrary}><BBInput aria-label="Sök i ditt bibliotek" onChange={event => setLibraryQuery(event.target.value)} placeholder="Sök i ditt bibliotek" value={libraryQuery} /><BBSelect aria-label="Sortera bibliotek" onChange={event => setLibrarySort(event.target.value)} value={librarySort}><option value="recent">Senast tillagda</option><option value="title">Titel</option><option value="author">Författare</option></BBSelect><BBButton busy={libraryBusy} type="submit">Filtrera biblioteket</BBButton></form>{visibleLibrary.length ? <><div className="audiobook-grid">{visibleLibrary.map(item => <Book item={item} key={item.id} onOpen={openDetail} />)}</div>{library.length < libraryTotal && <BBButton busy={libraryBusy} onClick={moreLibrary} variant="tertiary">Visa fler ljudböcker</BBButton>}</> : <BBEmptyState title="Biblioteket är tomt" />}</section>{showScrollTop && <BBButton aria-label="Till början av ljudboksbiblioteket" className="audiobook-scroll-top" onClick={scrollToTop} variant="secondary">↑</BBButton>}{selectedCandidate && <div aria-labelledby="audiobook-confirm-title" aria-modal="true" className="bb-dialog-backdrop" role="dialog"><BBSurface className="audiobook-detail"><h2 id="audiobook-confirm-title">{selectedCandidate.title}</h2><p>Lägg till den här utgåvan?</p><div className="bb-action-group"><BBButton onClick={() => setSelectedCandidate(null)} variant="tertiary">Avbryt</BBButton><BBButton busy={busy} disabled={!provider?.canRequest} onClick={() => add(selectedCandidate)} variant="primary">Lägg till</BBButton></div></BBSurface></div>}</section>
}
