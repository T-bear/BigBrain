...
export function Audiobooks() {
  ...
  if (route.kind === 'overview') return <section aria-labelledby="audiobooks-heading" className="audiobooks-section audiobook-overview"><a aria-label="Öppna ljudboksbiblioteket" className="audiobook-overview__heading audiobook-collection-link" href={collectionPath} onClick={event => { event.preventDefault(); navigate(collectionPath, { kind: 'collection' }) }}><span><span className="eyebrow">Lyssna</span><span className="audiobook-overview__title" id="audiobooks-heading">Ljudböcker</span></span><span>Bibliotek <span aria-hidden="true">›</span></span></a>{!data && <BBLoadingIndicator label="Laddar ljudböcker" />}{data?.state === 'notConfigured' && <BBEmptyState title="Audiobookshelf väntar på konfigurering" />}{data?.state === 'configuredUnavailable' && <BBEmptyState title="Ljudböcker är tillfälligt otillgängliga" detail={data.message ?? undefined} />}{data?.continueListening ? <BBSurface className="audiobook-continue-strip"><h3>Fortsätt lyssna</h3><CompactBook item={data.continueListening} onOpen={openDetail} /></BBSurface> : data?.state === 'configuredHealthy' && <p className="audiobook-overview__quiet">Fortsätt lyssna är inte tillgängligt för den anslutna ljudboksprofilen.</p>}</section>

  if (route.kind === 'detail') return <section className={`audiobooks-section audiobook-route-view audiobook-route-view--${direction}`}><BBButton className="audiobook-route-back" onClick={() => goBack(collectionPath, { kind: 'collection' })} variant="tertiary">‹ Ljudböcker</BBButton>{detailBusy && <BBLoadingIndicator label="Öppnar ljudbok" />}{selected && <article className="audiobook-detail-page"><BBMediaArtwork alt={`Omslag till ${selected.title}`} className="audiobook-detail-page__artwork" loading="eager" src={selected.coverUrl ?? undefined} /><div className="audiobook-detail-page__summary"><p className="eyebrow">Ljudbok</p><h1 ref={headingRef} tabIndex={-1}>{selected.title}</h1>{selected.author && <p className="audiobook-detail-page__author">{selected.author}</p>}<div className="audiobook-detail-page__metadata">{selected.series && <span>Serie: {selected.series}</span>}{selected.narrator && <span>Uppläsare: {selected.narrator}</span>}{knownLanguage(selected) && <span>{knownLanguage(selected)}</span>}{selected.publishedYear && <span>{selected.publishedYear}</span>}</div></div><div className="audiobook-detail-page__body">{selected.progressPercent !== null && <progress aria-label={`Lyssnat ${Math.round(selected.progressPercent)} procent`} max="100" value={selected.progressPercent} />}{selected.description && <p>{selected.description}</p>}{selected.playbackUrl && <a className="bb-button bb-button--secondary" href={selected.playbackUrl} rel="noreferrer">Öppna tillfälligt i Audiobookshelf</a>}</div></article>}{!selected && !detailBusy && notice && <BBEmptyState title={notice} />}</section>
