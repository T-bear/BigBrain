...
export function SleepTimerControl({ active = false, available = true, compact = false, label = 'Sovtimer', onActivate, onCancel, onClock, onMinutes }: Props) {
  ...
  return <div className={`audiobook-sleep-timer${compact ? ' audiobook-sleep-timer--compact' : ''}`}>
    <BBButton aria-controls={menuId} aria-expanded={open} aria-haspopup="dialog" aria-label={label} className="audiobook-sleep-timer__toggle" onClick={() => setOpen(value => !value)} ref={triggerRef} type="button" variant="tertiary"><AppIcon name="moon" size={23} />{!compact && <span>Sovtimer</span>}</BBButton>
    {open && <div aria-label="Sovtimeralternativ" className="audiobook-sleep-timer__panel" id={menuId} ref={panelRef} role="dialog">
      <header><strong>Sovtimer</strong><BBButton aria-label="Stäng sovtimer" onClick={() => { setOpen(false); triggerRef.current?.focus() }} type="button" variant="icon">×</BBButton></header>
