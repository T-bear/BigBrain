...
test('starts native playback directly while the non-control area retains detail navigation',async()=>{
  ...
  expect(continueTimer).toHaveAttribute('aria-expanded','false');expect(continueTimer).toHaveAttribute('aria-pressed','false');fireEvent.click(continueTimer);expect(screen.getByRole('dialog',{name:'Sovtimeralternativ'})).toBeVisible()
