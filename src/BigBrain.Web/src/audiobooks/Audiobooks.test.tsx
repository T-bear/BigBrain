...
test('uses progress-backed listening continuity and real collection/detail routes',async()=>{
  ...
  expect(screen.getByRole('button',{name:'Sovtimer för Påbörjad bok'})).toBeDisabled()
...
test('starts native playback directly while the non-control area retains detail navigation',async()=>{
  ...
  const continueTimer=screen.getByRole('button',{name:'Sovtimer för Påbörjad bok'});expect(continueTimer).toBeEnabled();expect(continueTimer).toHaveAttribute('aria-expanded','false')
  fireEvent.click(continueTimer);expect(continueTimer).toHaveAttribute('aria-expanded','true')
  fireEvent.change(screen.getByLabelText('Sovtimer'),{target:{value:'15'}});expect(screen.getByText(/15 min kvar/)).toBeInTheDocument()
  ...
  const detailTimer=screen.getByRole('button',{name:'Sovtimer'});if(detailTimer.getAttribute('aria-expanded')==='false')fireEvent.click(detailTimer);const sleepTimer=screen.getByRole('combobox',{name:'Sovtimer'})
