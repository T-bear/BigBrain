...
test('starts native playback directly while the non-control area retains detail navigation',async()=>{
  ...
  expect(continueTimer.closest('details')).toHaveAttribute('open');expect(screen.getByRole('combobox',{name:'Sovtimer'})).toBeVisible()
  ...
  const detailTimer=screen.getByRole('button',{name:'Sovtimer'});if(!detailTimer.closest('details')?.hasAttribute('open'))fireEvent.click(detailTimer);const sleepTimer=screen.getByRole('combobox',{name:'Sovtimer'})
