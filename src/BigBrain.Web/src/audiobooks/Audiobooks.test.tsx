...
const provider = {
  ...
  canRequest: false,
  canCancel: false,
  message: 'Ingen granskad anskaffningsleverantör är konfigurerad.',
}
...
function initial(
  ...
  value: unknown = overview,
  availability: unknown = playbackHealthy,
) {
  fetch
...
test('is registered as a first-class Media view', () => {
  const registry = createAppWidgetRegistry({
    modules: [],
    moduleError: false,
    system: null,
    systemError: false,
  ...
  expect(registry.getAll().some(widget => widget.id === 'audiobooks' && widget.defaultView === 'media')).toBe(true)
})
...
test('renders provider-neutral search with Swedish preference and no fake progress', async () => {
  ...
  render(<Audiobooks />)
  expect(await screen.findByText('Biblioteket är tomt')).toBeInTheDocument()
...
test('shows resolved canonical metadata separately from audiobook releases', async () => {
  ...
  initial(fetch)
  const metadata = {
    query: { original: 'The Wandering Inn', normalized: 'The Wandering Inn', kind: 'freeText' },
    state: 'resolved',
    narratorSearchSupported: false,
    message: null,
    works: [
      {
        workId: 'OL1W',
        editionIds: ['OL1M'],
        canonicalTitle: 'The Wandering Inn',
        alternateTitles: ['Wandering Inn'],
        authors: ['pirateaba'],
  ...
  render(<Audiobooks />)
  await screen.findByText('Biblioteket är tomt')
  fireEvent.change(screen.getByLabelText('Sök efter en ny ljudbok'), { target: { value: 'The Wandering Inn' } })
  fireEvent.click(screen.getByRole('button', { name: 'Sök' }))
...
test('keeps materially different discovery editions visible', async () => {
  ...
  const fetch = vi.spyOn(globalThis, 'fetch')
  initial(fetch)
  const candidate = (editionId: string, narrator: string, language: string, languageLabel: string) => ({
    workId: 'work',
    editionId,
    title: 'Boken',
    author: 'Författaren',
    ...
    provenance: 'AudioBookBay',
  })
  fetch.mockResolvedValueOnce(
    json({
      library: [],
      discovery: [candidate('sv-a', 'Röst A', 'sv', 'Svenska'), candidate('en-b', 'Voice B', 'en', 'Engelska')],
      acquisition: provider,
    }),
  )
  render(<Audiobooks />)
  ...
  fireEvent.change(screen.getByLabelText('Sök efter en ny ljudbok'), { target: { value: 'bok' } })
  fireEvent.click(screen.getByRole('button', { name: 'Sök' }))
  expect(await screen.findByText('2 utgåvor')).toBeInTheDocument()
  expect(screen.getByText(/Röst A/)).toBeInTheDocument()
...
test('shows truthful Audiobookshelf not-configured state', async () => {
  ...
  render(<Audiobooks />)
  expect(await screen.findByText('Audiobookshelf väntar på konfigurering')).toBeInTheDocument()
})
...
test('keeps library usable while acquisition provider is unavailable', async () => {
  ...
  const unavailable = {
    ...
    provider: 'librarr',
    message: 'Librarr kunde inte nås.',
  }
  const fetch = vi.spyOn(globalThis, 'fetch')
  fetch
    .mockResolvedValueOnce(json({ ...overview, acquisition: unavailable }))
    .mockResolvedValueOnce(json(playbackHealthy))
...
test('polls real provider state for active jobs without inventing progress', async () => {
  ...
  fetch
    ...
    .mockResolvedValueOnce(
      json({ ...provider, state: 'configuredHealthy', provider: 'librarr', canSearch: true, canRequest: true }),
    )
    .mockResolvedValueOnce(json({ ...jobs, items: [active], total: 1 }))
    .mockResolvedValueOnce(json({ items: [], page: 0, pageSize: 24, total: 0, hasMore: false }))
    .mockResolvedValueOnce(json({ ...active, status: 'downloading' }))
  render(<Audiobooks />)
...
test('requires explicit edition confirmation before creating one acquisition job', async () => {
  ...
  const candidate = {
    workId: 'work',
    editionId: 'edition',
    title: 'En mycket lång ljudboksutgåva',
    author: 'Författaren',
    narrator: 'Röst',
    language: 'sv',
    languageLabel: 'Svenska',
    edition: 'Oavkortad · 1.2 GB',
    durationSeconds: null,
    publicationYear: 2025,
    coverUrl: null,
    source: 'librarr',
  ...
  const fetch = vi.spyOn(globalThis, 'fetch')
  fetch
    .mockResolvedValueOnce(json({ ...overview, acquisition: healthy }))
    .mockResolvedValueOnce(json(playbackHealthy))
  ...
  render(<Audiobooks />)
  await screen.findByText('Biblioteket är tomt')
  fireEvent.change(screen.getByLabelText('Sök efter en ny ljudbok'), { target: { value: 'ljudbok' } })
  fireEvent.click(screen.getByRole('button', { name: 'Sök' }))
  await screen.findByText('1 utgåvor')
  expect(fetch.mock.calls.filter(([, init]) => (init as RequestInit | undefined)?.method === 'POST')).toHaveLength(0)
  fireEvent.click(screen.getByRole('button', { name: 'Välj utgåva' }))
  expect(screen.getByRole('dialog')).toHaveTextContent('Lägg till den här utgåvan?')
  fireEvent.click(screen.getByRole('button', { name: 'Lägg till' }))
  await waitFor(() =>
    expect(fetch.mock.calls.filter(([, init]) => (init as RequestInit | undefined)?.method === 'POST')).toHaveLength(1),
  )
...
test('refreshes Audiobookshelf library after indexing is confirmed complete', async () => {
  ...
  const active = {
    ...
    candidate: {
      workId: 'work',
      editionId: 'edition',
      title: 'Hämtad release',
      author: 'Författaren',
      narrator: null,
      language: 'und',
      languageLabel: 'Språk okänt',
      edition: 'Release',
      durationSeconds: null,
      publicationYear: null,
      coverUrl: null,
    ...
    createdAtUtc: '2026-08-24T10:00:00Z',
    updatedAtUtc: '2026-08-24T10:00:00Z',
    message: 'Väntar på indexering.',
  }
  const imported = {
    id: 'abs-item',
    title: 'Importerad bok',
    author: 'Författaren',
    series: null,
    narrator: null,
    language: 'und',
    ...
    publishedYear: null,
    isAbridged: null,
    playbackUrl: 'http://owner/item/abs-item',
  }
  const fetch = vi.spyOn(globalThis, 'fetch')
  fetch
    .mockResolvedValueOnce(json(overview))
    ...
    .mockResolvedValueOnce(json({ ...jobs, items: [active], total: 1 }))
    .mockResolvedValueOnce(json({ items: [], page: 0, pageSize: 24, total: 0, hasMore: false }))
    .mockResolvedValueOnce(json({ ...active, status: 'completed', message: null }))
    .mockResolvedValueOnce(json({ ...overview, library: [imported], recent: [imported] }))
  render(<Audiobooks />)
  expect(await screen.findByText('Importerad bok')).toBeInTheDocument()
...
test('keeps completed acquisition history collapsed while active work stays visible', async () => {
  ...
  const done = {
    ...active,
    id: 'b'.repeat(32),
    candidate: { ...active.candidate, editionId: 'done', title: 'Gammal bok' },
    status: 'completed',
  }
  const fetch = vi.spyOn(globalThis, 'fetch')
  fetch
    ...
    .mockResolvedValueOnce(json(playbackHealthy))
    .mockResolvedValueOnce(json(provider))
    .mockResolvedValueOnce(json({ ...jobs, items: [active, done], total: 42 }))
  render(<Audiobooks />)
  expect(await screen.findByText('Pågående bok')).toBeVisible()
  expect(screen.getByText('Historik (41)')).toBeInTheDocument()
  expect(screen.getByText('Gammal bok').closest('details')).not.toHaveAttribute('open')
})
...
test('dismisses failed attention locally without mutating audit or provider state', async () => {
  ...
  const failed = {
    ...
    createdAtUtc: '2026-08-27T10:00:00Z',
    updatedAtUtc: '2026-08-27T10:00:00Z',
    message: 'Jobbet finns inte längre hos leverantören.',
  }
  const fetch = vi.spyOn(globalThis, 'fetch')
  fetch
    .mockResolvedValueOnce(json(overview))
    .mockResolvedValueOnce(json(playbackHealthy))
    .mockResolvedValueOnce(json(provider))
    .mockResolvedValueOnce(json({ ...jobs, items: [failed], total: 1 }))
    .mockResolvedValueOnce(json({ items: [], page: 0, pageSize: 24, total: 0, hasMore: false }))
  render(<Audiobooks />)
  await screen.findByText('Gammalt misslyckat jobb')
  ...
  expect(JSON.parse(window.localStorage.getItem('bigbrain.audiobooks.hidden-attention.v1') ?? '[]')).toEqual([
    failed.id,
  ])
  expect(
...
test('keeps the Media overview compact and opens the bounded full collection on demand', async () => {
  ...
  const fetch = vi.spyOn(globalThis, 'fetch')
  initial(fetch, { ...overview, library: books, recent: books.slice(0, 6) })
  render(<Audiobooks />)
  const libraryLink = await screen.findByRole('link', { name: 'Öppna ljudboksbiblioteket' })
  expect(libraryLink).toHaveAttribute('href', '/media/audiobooks')
  expect(libraryLink).not.toHaveClass('bb-button')
...
test('uses progress-backed listening continuity and real collection/detail routes', async () => {
  const listening = {
    ...
    publishedYear: '2025',
    isAbridged: false,
    playbackUrl: 'https://owner.example/item/book-1',
  }
  const fetch = vi.spyOn(globalThis, 'fetch')
  initial(fetch, { ...overview, continueListening: listening, library: [listening], recent: [listening] })
  render(<Audiobooks />)
  expect(await screen.findByText('Fortsätt lyssna')).toBeInTheDocument()
  ...
  const timer = screen.getByRole('button', { name: 'Sovtimer för Påbörjad bok' })
  expect(timer).toBeEnabled()
  fireEvent.click(timer)
  expect(screen.getByText('Starta uppspelningen för att använda sovtimern.')).toBeVisible()
  expect(screen.getByRole('button', { name: 'Starta uppspelningen' })).toBeVisible()
...
test('starts native playback directly while the non-control area retains detail navigation', async () => {
      ...
      tracks: [
        {
          ...
          title: null,
          mimeType: 'audio/mpeg',
          streamUrl: '/api/v1/modules/media/audiobooks/playback/sessions/session/tracks/0',
        },
      ],
      expiresAtUtc: '2026-08-29T20:00:00Z',
    }),
  )
  const mediaPlay = vi.spyOn(HTMLMediaElement.prototype, 'play').mockImplementation(async function (
  ...
  const navigation = await screen.findByRole('button', { name: 'Öppna Påbörjad bok' })
  const play = screen.getByRole('button', { name: 'Spela Påbörjad bok' })
  expect(play).toHaveAttribute('aria-pressed', 'false')
  const continueTimer = screen.getByRole('button', { name: 'Sovtimer för Påbörjad bok' })
  fireEvent.click(continueTimer)
  expect(screen.getByText('Starta uppspelningen för att använda sovtimern.')).toBeVisible()
  await waitFor(() => expect(screen.getByRole('button', { name: 'Stäng sovtimer' })).toHaveFocus())
  ...
  expect(
    fetch.mock.calls.some(
      ([url, init]) => String(url).endsWith('/book-1/playback') && (init as RequestInit).method === 'POST',
    ),
  ).toBe(true)
  expect(window.location.pathname).toBe('/')
  expect(continueTimer).toHaveAttribute('aria-expanded', 'false')
  expect(continueTimer).toHaveAttribute('aria-pressed', 'false')
  fireEvent.click(continueTimer)
  expect(screen.getByRole('dialog', { name: 'Sovtimeralternativ' })).toBeVisible()
  fireEvent.click(screen.getByRole('button', { name: '15 min' }))
  expect(screen.getByText(/15 min kvar/)).toBeInTheDocument()
  expect(continueTimer).toHaveAttribute('aria-pressed', 'true')
  fireEvent.click(navigation)
  expect(window.location.pathname).toBe('/media/audiobooks/book-1')
  expect(screen.getByRole('region', { name: 'Spelar Påbörjad bok' })).toBeInTheDocument()
  expect(screen.getByText(/15 min kvar/)).toBeInTheDocument()
  const detailTimer = screen.getByRole('button', { name: 'Sovtimer' })
  fireEvent.click(detailTimer)
  fireEvent.click(screen.getByRole('button', { name: 'Sluttid…' }))
  fireEvent.change(screen.getByLabelText('Välj lokal sluttid'), { target: { value: '22:30' } })
  expect(screen.getByText(/Stannar 22:30/)).toBeInTheDocument()
  fireEvent.click(detailTimer)
  fireEvent.click(screen.getByRole('button', { name: 'Stäng av' }))
  expect(screen.queryByText(/min kvar/)).not.toBeInTheDocument()
  fireEvent.click(detailTimer)
  fireEvent.click(screen.getByRole('button', { name: '15 min' }))
  ...
  act(() => {
    if (typeof expiration === 'function') expiration()
  })
...
test('shows a truthful fallback instead of a native primary action when playback is unavailable', async () => {
  ...
  const item = {
    ...
    publishedYear: null,
    isAbridged: null,
    playbackUrl: 'https://owner.example/item/unplayable',
  }
  const unavailable = {
    state: 'configuredUnavailable',
    message: 'Playback-identiteten kunde inte verifieras.',
    separateIdentity: false,
    hasProgress: false,
  }
  const fetch = vi.spyOn(globalThis, 'fetch')
  initial(fetch, overview, unavailable)
  fetch.mockResolvedValueOnce(json(item))
  render(<Audiobooks />)
  expect(await screen.findByText('BigBrains spelare är inte tillgänglig för den här ljudboken.')).toBeInTheDocument()
...
test('presents semantic detail metadata without raw filler', async () => {
  ...
  const item = {
    id: 'ghostsong',
    title: 'Ghostsong (Unabridged)',
    author: 'Pirateaba',
    series: 'Singer of Terandria Series #3',
    narrator: 'Andrea Parsneau',
    language: 'en',
    languageLabel: 'Engelska',
    durationSeconds: 1000,
    progressPercent: null,
    description: 'Användbar beskrivning.',
    coverUrl: null,
    publishedYear: '2025',
    isAbridged: false,
    playbackUrl: 'https://owner.example/item/ghostsong',
  }
  const fetch = vi.spyOn(globalThis, 'fetch')
  initial(fetch)
  fetch.mockResolvedValueOnce(json(item))
  render(<Audiobooks />)
  expect(await screen.findByText('Av Pirateaba')).toBeInTheDocument()
...
test('loads a deep-linked audiobook detail through the bounded BigBrain API', async () => {
  ...
  fetch.mockResolvedValueOnce(json(item))
  render(<Audiobooks />)
  expect(await screen.findByRole('heading', { name: 'Direktlänkad bok' })).toBeInTheDocument()
  ...
  expect(screen.getByRole('img', { name: 'Omslag till Direktlänkad bok' })).toHaveClass(
    'audiobook-detail-page__artwork',
  )
})
...
test('starts a forward detail route at top and restores collection scroll on back', async () => {
  ...
  const fetch = vi.spyOn(globalThis, 'fetch')
  initial(fetch, { ...overview, library: [item], recent: [item] })
  ...
  render(<Audiobooks />)
  fireEvent.click(await screen.findByRole('button', { name: 'Öppna Scrollbok' }))
...
test('restores collection query and sort after returning from detail', async () => {
  ...
  initial(fetch, { ...overview, library: [item], recent: [item] })
  render(<Audiobooks />)
  await screen.findByText('En bok')
...
test('uses the whole semantic book row for detail navigation', async () => {
  ...
  initial(fetch, { ...overview, library: [item], recent: [item] })
  render(<Audiobooks />)
  const row = await screen.findByRole('button', { name: 'Öppna Tryckbar bok' })
  expect(row).toHaveClass('audiobook-book-row')
  expect(screen.queryByText('Visa ljudbok')).not.toBeInTheDocument()
  fireEvent.click(row)
  expect(window.location.pathname).toBe('/media/audiobooks/whole-row')
})
...
test('orders discovery before library before downloads', async () => {
  ...
  render(<Audiobooks />)
  await screen.findByText('Biblioteket är tomt')
  const discovery = screen.getByRole('heading', { name: 'Hitta ljudbok' })
  const library = screen.getByRole('heading', { name: 'Bibliotek' })
  const downloads = await screen.findByRole('heading', { name: 'Hämtningar' })
