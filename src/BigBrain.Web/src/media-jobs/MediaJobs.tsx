...
function compactTitle(title: string) {
  const normalized = title.match(
    /^(.*?)\s+((?:19|20)\d{2})\s+(?=(?:EXTRAS|2160p|1080p|720p|480p|UHD|BluRay|WEB[- .]?DL|WEBRip|HDTV)\b)/i,
  )
...
export function MediaJobs({ showHeading = true }: { showHeading?: boolean }) {
  ...
  const unavailableProviders = (Array.isArray(snapshot?.providers) ? snapshot.providers : []).filter(
    provider => provider.status !== 'online',
  )

  return (
    <section
      className="media-jobs-section"
      aria-label={showHeading ? undefined : 'Poster i Medieflödet'}
      aria-labelledby={showHeading ? 'media-jobs-heading' : undefined}
    >
      {showHeading && (
        <header className="media-jobs-heading">
          <div>
            <p className="eyebrow">Live activity</p>
            <h3 id="media-jobs-heading">Media Jobs</h3>
          </div>
          <span aria-live="polite">{failed ? 'Uppdatering avbruten' : 'Uppdateras automatiskt'}</span>
        </header>
      )}
      {!showHeading && (
        <p className="media-jobs-update-status" aria-live="polite">
          {failed ? 'Uppdatering avbruten' : 'Uppdateras automatiskt'}
        </p>
      )}
      {!snapshot && !failed && <p aria-live="polite">Loading media jobs…</p>}
      {failed && !snapshot && (
        <p role="alert" className="notice notice--error">
          Media jobs could not be loaded.
        </p>
      )}
      {failed && snapshot && (
        <p role="status" className="notice notice--warning">
          Automatisk uppdatering är tillfälligt otillgänglig. Senaste status visas.
        </p>
      )}
      ...
      {compactActive && (
        <section aria-labelledby="media-jobs-compact-heading" className="media-jobs-compact">
          <header>
            <h4 id="media-jobs-compact-heading">{jobs.length} pågående nedladdningar</h4>
            <button
              aria-controls="media-jobs-expanded"
              aria-expanded={jobsExpanded}
              className="secondary-button"
              onClick={() => setJobsExpanded(value => !value)}
              type="button"
            >
              {jobsExpanded ? 'Dölj nedladdningar' : 'Visa nedladdningar'}
            </button>
          </header>
          <ul>
            {visibleJobs.map(job => (
              <li key={job.id}>
                <span>
                  <strong title={job.title}>{compactTitle(job.title)}</strong>
                  <small>{compactStatus[job.status] ?? job.status}</small>
                </span>
                {job.progressPercent !== null && <b>{Math.max(0, Math.min(100, job.progressPercent)).toFixed(0)}%</b>}
              </li>
            ))}
          </ul>
        </section>
      ...
      {jobs.length > 0 && (!compactActive || jobsExpanded) && (
        <div className="media-jobs-grid" id={compactActive ? 'media-jobs-expanded' : undefined}>
          {visibleJobs.map(job => (
            <MediaJobCard job={job} key={job.id} />
          ))}
        </div>
      )}
      {visibleCount < jobs.length && (
        <button
          className="secondary-button media-jobs-more"
          type="button"
