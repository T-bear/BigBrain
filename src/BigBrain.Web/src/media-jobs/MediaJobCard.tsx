...
const mediaTypeLabels: Record<MediaJob['mediaType'], string> = {
  movie: 'Film',
  series: 'Serie',
  season: 'Säsong',
  episode: 'Avsnitt',
...
export function MediaJobCard({ job }: { job: MediaJob }) {
  ...
  return (
    <article className={`media-job media-job--${job.status}`}>
      <header>
      ...
      {progress !== null && job.status !== 'available' && (
        <progress max="100" value={progress} aria-label={`${job.title} progress`}>
          {progress}%
        </progress>
      )}
      <div className="media-job__metrics">
        {job.episodeCount !== null && (
          <span>
            {job.completedEpisodeCount ?? 0} av {job.episodeCount} avsnitt klara
          </span>
        )}
        {eta(job.etaSeconds) && <span>Cirka {eta(job.etaSeconds)} kvar</span>}
      </div>
      {job.userMessage && (
        <p role={job.status === 'failed' ? 'alert' : 'status'} className="media-job__error">
          {job.userMessage}
        </p>
      )}
      <details className="media-job__details">
        <summary>Visa tekniska detaljer</summary>
        <div className="media-job__technical-summary">
          <span>Provider: {job.provider}</span>
          {job.subtitle && <span>Fullständig titel: {job.subtitle}</span>}
          <span>Intern status: {job.status}</span>
        </div>
            ...
            {job.details.map((detail, index) => (
              <li key={`${detail.provider}:${detail.subtitle ?? index}`}>
                ...
                <span>
                  ...
                  {detail.progressPercent === null ? '' : ` · ${detail.progressPercent.toFixed(0)}%`}
                </span>
      ...
      {play?.canPlay && (
        <a className="primary-button media-job__play" href={play.playUrl}>
          ▶ Spela i Jellyfin
        </a>
      )}
    </article>
