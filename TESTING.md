...
- Build `bigbrain-librarr:1208254-bb4`; the image gate runs complete upstream organizer/search/download tests. Regressions prove an existing audiobook destination creates durable failure evidence, never deletes the qBittorrent job/source and never changes existing content. Discovery tests cover unchanged title-only behavior, bounded title-plus-author variants reaching sources, normalized deduplication, audiobook-suffix suppression and retained partial-source results; existing scoring/language tests remain in the complete search package.
