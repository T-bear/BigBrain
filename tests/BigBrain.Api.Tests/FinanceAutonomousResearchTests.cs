...
public sealed class FinanceAutonomousResearchTests
{
    ...
    private static ResearchFixture Ready()
    {
        ...
        for (var i = 0; i < 280; i++) { while (date.DayOfWeek is DayOfWeek.Saturday or DayOfWeek.Sunday) date = date.AddDays(1); var close = 100m + i * .1m + (i % 17 - 8) * .2m; bars.Add(new(date, close - .1m, close + .5m, close - .5m, close, close, 1000 + i)); date = date.AddDays(1); }
        var acquired = new DateTimeOffset(2026, 8, 21, 22, 0, 0, TimeSpan.Zero); memory.Store(instrument, bars, System.Text.Encoding.UTF8.GetBytes("bb092-remediation-fixture"), bars[0].Date, bars[^1].Date, acquired.AddMinutes(-1), acquired, 0); memory.BuildFeatures(); return new(root, options, memory);
    }
    private static FinanceFeatureBuildEvidence AdvanceGeneration(ResearchFixture fixture, string identity, decimal offset)
    {
        ...
        for (var i = 0; i < 280; i++) { while (date.DayOfWeek is DayOfWeek.Saturday or DayOfWeek.Sunday) date = date.AddDays(1); var close = 100m + offset + i * .1m + (i % 17 - 8) * .2m; bars.Add(new(date, close - .1m, close + .5m, close - .5m, close, close, 2000 + i)); date = date.AddDays(1); }
        var acquired = new DateTimeOffset(2026, 8, 22, 12, 0, 0, TimeSpan.Zero); fixture.Memory.Store(instrument, bars, System.Text.Encoding.UTF8.GetBytes(identity), bars[0].Date, bars[^1].Date, acquired.AddMinutes(-1), acquired, 0); return fixture.Memory.BuildFeatures();
    }
