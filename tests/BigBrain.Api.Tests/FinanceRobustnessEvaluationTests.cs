...
public sealed class FinanceRobustnessEvaluationTests
{
    ...
    private static (BacktestMarketBar[] Bars, BacktestFeatureValue[] Features) Fixture(int sessions, bool futureKnowledge = false)
    {
        ...
        {
            var date = start.AddDays(i); var price = 100m + i * .1m + (i % 17 - 8) * .2m; var knowledge = new DateTimeOffset(date.ToDateTime(new TimeOnly(22, 0)), TimeSpan.Zero); bars.Add(new(new("US:XNAS:TEST"), "market-1", date, price, price, knowledge));
