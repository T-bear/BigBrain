...
public sealed class FinanceDatasetIntakeTests
{
    ...
    private sealed class IntakeFixture : IDisposable
    {
        ...
        internal string Write(string name, string content) { Directory.CreateDirectory(_root); var path = Path.Combine(_root, name); File.WriteAllText(path, content); return path; }
