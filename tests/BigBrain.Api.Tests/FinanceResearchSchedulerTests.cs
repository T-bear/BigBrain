...
public sealed class FinanceResearchSchedulerTests
{
    ...
    {
        using var fixture = Ready(); var options = Enabled(); var orchestrator = Orchestrator(options, fixture.Memory); var now = Due(fixture); var riskPolicy = fixture.Memory.RiskPolicySnapshot();
        ...
        var reopened = new EodhdMarketMemory(fixture.Options); var afterRestart = Orchestrator(options, reopened).CheckAndRun(now.AddMinutes(10), true, CancellationToken.None); Assert.Equal(first.ResearchRunId, afterRestart!.ResearchRunId); Assert.Equal(2, reopened.AutonomousResearchSnapshot().TotalExperiments);
    }

    [Fact]
    public void MissedDaysCreateOnlyCurrentOpportunityWithoutCatchUpStorm()
    {
        using var fixture = Ready(); var orchestrator = Orchestrator(Enabled(), fixture.Memory); var now = Due(fixture).AddDays(10); var result = orchestrator.CheckAndRun(now, true, CancellationToken.None); var history = fixture.Memory.ResearchSchedulerHistory(0, 100);
    ...
    public void CurrentEvidenceUnavailableIsDurableAndDoesNotRetryOrCreateExperiments()
    {
        ...
        Assert.Equal(FinanceResearchOpportunityState.Failed, failed!.State); Assert.Equal("finance.research.currentEvidenceIncomplete", failed.Reason); Assert.Equal(failed.OpportunityId, repeated!.OpportunityId); Assert.Equal(failed.State, repeated.State); Assert.Equal(failed.ResearchRunId, repeated.ResearchRunId); Assert.Equal(failed.ResourceDecision!.Decision, repeated.ResourceDecision!.Decision); Assert.Equal(failed.ResourceDecision.ReasonCodes, repeated.ResourceDecision.ReasonCodes); Assert.Empty(fixture.Memory.ResearchExperiments(0, 10, null, null, null, null, null).Experiments); Assert.Equal(ResearchExperimentState.Failed, fixture.Memory.ResearchRun(failed.ResearchRunId!)!.State);
    }

    [Fact]
    public void RecoveryAndDataReadinessDeferUntilNextBoundedCheck()
    {
        using var fixture = Ready(); var options = Enabled(); var orchestrator = Orchestrator(options, fixture.Memory); var now = Due(fixture); var recovery = orchestrator.CheckAndRun(now, false, CancellationToken.None);
    ...
    public void RestartReconcilesCompletedResearchBeforeJournalCompletion()
    {
        ...
        var reopened = new EodhdMarketMemory(fixture.Options); var reconciled = Orchestrator(options, reopened).CheckAndRun(now.AddMinutes(2), true, CancellationToken.None); Assert.Equal(FinanceResearchOpportunityState.Completed, reconciled!.State); Assert.Equal(run.RunId, reconciled.ResearchRunId); Assert.Equal(2, reopened.AutonomousResearchSnapshot().TotalExperiments);
    }
    ...
    public async Task DisabledDefaultAndCancellationNeverCreateResearchOrFailure()
    {
        ...
        using var cancelled = new CancellationTokenSource(); cancelled.Cancel(); Assert.Throws<OperationCanceledException>(() => Orchestrator(Enabled(), fixture.Memory).CheckAndRun(Due(fixture), true, cancelled.Token)); Assert.Empty(fixture.Memory.ResearchSchedulerHistory(0, 10).Opportunities);
    }
    ...
    public void ConfigurationBoundsAndStatusSafetyAreExplicit()
    {
        ...
        using var fixture = Ready(); var status = Orchestrator(Enabled(), fixture.Memory).Status(Due(fixture)); Assert.Equal("RESEARCH", status.OperatingMode); Assert.Equal(0m, status.BudgetSek); Assert.Equal("NONE", status.ExecutionAuthority); Assert.False(status.ResearchCurrentlyRunning);
    }
    ...
    public void StatusExposesCompleteUniverseAndExactFeatureLineageForEligibleSession()
    {
        ...
        using (var c = Open(fixture)) { using var x = c.CreateCommand(); x.CommandText = "UPDATE feature_revisions SET source_revisions_json='[]'"; x.ExecuteNonQuery(); }
        var incompatible = Orchestrator(Enabled(), fixture.Memory).Status(Due(fixture)); Assert.Equal("COMPLETE", incompatible.CurrentSessionReadiness); Assert.Equal("INCOMPATIBLE", incompatible.FeatureLineageReadiness); Assert.False(incompatible.DataReady);
    }
    ...
    public void PartialUniverseDefersWithoutResearchThenSameOpportunityProceedsWhenCoherent()
    {
        ...
        var options = Enabled(); var orchestrator = Orchestrator(options, fixture.Memory); var now = Due(fixture); var deferred = orchestrator.CheckAndRun(now, true, CancellationToken.None);
    ...
    public void CurrentMarketRequiresMatchingCurrentFeatureGeneration()
    {
        ...
        var options = Enabled(); var orchestrator = Orchestrator(options, fixture.Memory); var now = new DateTimeOffset(next.AddDays(1), new TimeOnly(3, 0), TimeSpan.Zero); var deferred = orchestrator.CheckAndRun(now, true, CancellationToken.None);
    ...
    public void FeatureLineageMismatchFailsClosedDeterministically()
    {
        ...
        var result = Orchestrator(Enabled(), fixture.Memory).CheckAndRun(Due(fixture), true, CancellationToken.None); Assert.Equal(FinanceResearchOpportunityState.Deferred, result!.State); Assert.Empty(fixture.Memory.ResearchRuns(0, 10).Runs);
    }
    ...
    {
        using var fixture = Ready(); using (var c = Open(fixture)) { using var x = c.CreateCommand(); x.CommandText = "DELETE FROM observations WHERE instrument_id='US:XNYS:XOM' AND session_date=$date"; x.Parameters.AddWithValue("$date", fixture.LastSession.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture)); x.ExecuteNonQuery(); }
        var orchestrator = Orchestrator(Enabled(), fixture.Memory); var first = orchestrator.CheckAndRun(Due(fixture), true, CancellationToken.None); Assert.Equal(FinanceResearchOpportunityState.Deferred, first!.State);
    ...
    [Fact]
    public async Task ResourcePressureDefersWithoutEvidenceAndSameOpportunityLaterRunsOnce()
    {
        using var fixture = Ready(); var now = Due(fixture); var pressure = Resource(FinanceResearchResourceDecisionKind.Defer, now, "finance.research.scheduler.resource.cpu"); var allowed = Resource(FinanceResearchResourceDecisionKind.Allow, now.AddHours(1), "finance.research.scheduler.resource.ready"); var governor = new ScriptedGovernor(pressure, allowed); var orchestrator = new FinanceResearchOrchestrator(Enabled(), fixture.Memory, governor);
    ...
    private static FinanceResearchOrchestrator Orchestrator(FinanceResearchSchedulerOptions options, EodhdMarketMemory memory) => new(options, memory, new ScriptedGovernor(Resource(FinanceResearchResourceDecisionKind.Allow, new DateTimeOffset(2026, 8, 23, 3, 0, 0, TimeSpan.Zero), "finance.research.scheduler.resource.ready")));
    ...
    private static Fixture Ready()
    {
        ...
        var acquired = new DateTimeOffset(2026, 8, 22, 12, 0, 0, TimeSpan.Zero); foreach (var instrument in EodhdCatalog.Watchlist) memory.Store(instrument, bars, System.Text.Encoding.UTF8.GetBytes("bb093-fixture-" + instrument.Symbol), bars[0].Date, bars[^1].Date, acquired.AddMinutes(-1), acquired, 0); memory.BuildFeatures(); return new(root, options, memory, bars[^1].Date, bars);
    }
    ...
    private static FinanceResearchResourceDecision Resource(FinanceResearchResourceDecisionKind kind, DateTimeOffset now, string reason) => new(kind, now, FinanceResearchResourceGovernorOptions.CurrentVersion, [reason], new(90, 50, 8L * 1024 * 1024 * 1024, 100L * 1024 * 1024 * 1024, 1, null, false, "Healthy", now), "RESEARCH", 0m, "NONE");
    ...
    private sealed class ScriptedGovernor(params FinanceResearchResourceDecision[] decisions) : IFinanceResearchResourceGovernor { private int index; public Task<FinanceResearchResourceDecision> EvaluateAsync(DateTimeOffset nowUtc, CancellationToken cancellationToken) => Task.FromResult(decisions[Math.Min(index++, decisions.Length - 1)]); }
}
