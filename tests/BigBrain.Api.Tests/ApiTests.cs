...
public sealed class ApiTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task ResearchSchedulerStatusIsDisabledByDefaultAndHistoryIsBounded()
    {
        ...
        Assert.Equal(HttpStatusCode.OK,status.StatusCode);var body=await status.Content.ReadAsStringAsync(TestContext.Current.CancellationToken);Assert.Contains("\"enabled\":false",body,StringComparison.Ordinal);Assert.Contains("\"historicalEvidenceAvailable\":false",body,StringComparison.Ordinal);Assert.Contains("\"currentSessionRequired\":",body,StringComparison.Ordinal);Assert.Contains("\"currentSessionReadiness\":",body,StringComparison.Ordinal);Assert.Contains("\"featureLineageReadiness\":",body,StringComparison.Ordinal);Assert.Contains("\"dataReady\":false",body,StringComparison.Ordinal);Assert.Contains("\"expectedInstrumentCount\":8",body,StringComparison.Ordinal);Assert.Contains("\"operatingMode\":\"RESEARCH\"",body,StringComparison.Ordinal);Assert.Contains("\"executionAuthority\":\"NONE\"",body,StringComparison.Ordinal);Assert.Equal(HttpStatusCode.OK,history.StatusCode);Assert.Equal(HttpStatusCode.BadRequest,invalid.StatusCode);
    }
