...
`BBButton` är den kanoniska action-primitiven med de begränsade varianterna primary, secondary, tertiary, icon, danger och contextual. Busy, disabled, pressed och focus ägs av komponent-/tokenkontraktet. En modul får komponera placering och bredd men ska inte ändra variantens färg, typografi eller kontrollgeometri. En semantisk collection-rad som primärt öppnar objektets detail är däremot en navigation row, inte en specialvariant av en CTA; den ska vara en riktig länk/knapp med gemensam focus/touch-state och objektets hela meningsfulla yta som target.
