...
**ACCEPTED / MERGED / CI VERIFIED**, architecture only, with [ADR 0038](../../adr/0038-finance-research-learning-authority-contract.md)
now Accepted without implementation authorization. Reuse existing Finance engines; AI proposes and never owns scientific/risk truth.
...
BB-090 completed on 2026-08-16: Finance SQLite has one ordered transactional migration coordinator with legacy bootstrap and lock serialization; Macro artifacts use quarantine/hash/rights/schema/validation/promotion evidence; and market revisions have explicit raw/adjusted capabilities. Production backup, isolated migration drill, deployment and restart idempotence passed. The owner-configured secret was activated safely and a bounded first-party CPIAUCSL/UNRATE vintage drill produced separate point-in-time revisions without altering revised history. Finalization commit `0f2ba9c3e7d5e7b3bf088c9094bdd79b3a35db1e` passed GitHub Actions run 55. BB-091, security/pentest, PAPER and later epics are not activated by this completion.

BB-091 completed on 2026-08-17. Its bounded Riksbank/ECB Macro and FX evidence provides region-aware as-of context for a later `FINANCE AUTONOMOUS RESEARCH v1`; it does not implement scheduling, feature proliferation, strategy selection or self-promotion. The preferred next Finance slice is the smallest deterministic autonomous-research orchestration foundation over existing evidence, predictions, maturations and evaluation gates—not another broad data pack.
