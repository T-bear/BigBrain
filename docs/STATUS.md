...
# BB-091 in progress (2026-08-17)

Provider-neutral Riksbank/ECB Macro and FX implementation is locally focused-test verified. Migration 93 adds region/unit/frequency and explicit base/quote metadata while preserving old FRED defaults. Official SWEA REST v1 and ECB SDMX 2.1 interfaces and reuse terms were checked; both selected first-party packs are `LOCAL_RESEARCH` with attribution required. Current-history bootstrap remains `REVISED_HISTORY_EXPLORATORY`. Production migration/acquisition, preservation/restart, full regression and published CI are not yet recorded, so BB-091 is not complete. Finance remains RESEARCH / 0 SEK with no PAPER, broker, order, LIVE/AUTO or self-learning.
