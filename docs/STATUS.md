...
BB-100 is implemented, published, CI-verified and foundation-deployed from baseline `c2efe5385f714de10490b2dee54db76055f66a70`. Media owns a bounded Audiobookshelf adapter and BigBrain read-only audiobook API, while `IAudiobookAcquisitionProvider` is truthfully `NotConfigured`. Official Readarr is retired and is not a dependency. Library, progress, continue-listening, secure cover proxy, local search, language filtering and detail UX are implemented without exposing the ABS API key to Web. Browser review found and corrected the initial placement in an unused `MediaDashboard` branch; `Ljudböcker` is now a first-class widget in the deployed Media route. Local validation passed 124 frontend, 506 API and 32 Sentinel tests, production/Release builds, documentation, Compose and diff checks. CI runs `32646849659`, `32647803354` and `32649796216` passed. Audiobookshelf 2.36.0, API and Web are running; ABS setup is loopback-only and BigBrain correctly reports `notConfigured` until the owner creates the library and dedicated API identity. All three themes passed 390 × 844 and 430 × 932 normal-viewport review without horizontal overflow. A final read-only appliance check found Sentinel restarting because `/run/bigbrain/sentinel.sock` is already in use; BB-100 did not change or restart that separate service, and runtime remediation was intentionally not performed outside this resumed frontend-only scope. Finance remains `RESEARCH / 0 SEK / NONE`; scheduler is enabled, idle and unchanged. BB-099 remains **TECHNICALLY COMPLETE / OWNER UX REVIEW PENDING**.
