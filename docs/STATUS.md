...
BB-100 is implemented locally from baseline `c2efe5385f714de10490b2dee54db76055f66a70`; publication, CI and appliance deployment evidence are pending. Media now owns a bounded Audiobookshelf adapter and BigBrain read-only audiobook API, while an `IAudiobookAcquisitionProvider` boundary is deployed as truthful `NotConfigured`. Official Readarr is retired and is not a dependency. Library, progress, continue-listening, cover proxy, local search, language filtering and detail UX are implemented without exposing the ABS API key to Web. Swedish is the stored default preference, English the fallback, and unknown language never aliases either. BB-099 remains **TECHNICALLY COMPLETE / OWNER UX REVIEW PENDING**.
