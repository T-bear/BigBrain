...
BB-128C is **IMPLEMENTED / AUTOMATICALLY VERIFIED / DEPLOYMENT PENDING / OWNER VISUAL REVIEW PENDING** from baseline `2fa43170f51f434e6adb04765618d88ffb74c601`. Finance initial loading and retry now consume the shared `BBLoadingIndicator` and `BBButton busy` contracts instead of swapping visible labels. The cached/degraded treatment is compact, retains an explicit stale/failure status, and renders label and fetch time as wrapping-safe elements without an orphanable literal separator. Shared reduced-motion and accessible busy semantics remain authoritative.
