...
BB-119 clarifies the read model without weakening that gate. Status first pins the scheduler date and asks `UsMarketCalendar` whether a session is required. A non-session date reports historical evidence independently and marks current-session and feature-lineage requirements `NOT_REQUIRED`; it must not query a fabricated Sunday/holiday session and report 0/8. When a session is required, the exact BB-093 universe and lineage checks above remain authoritative. Operations consumes the same current projection rather than interpreting the most recent durable opportunity reason as data readiness.
