...
**Final evidence-selection remediation (2026-08-22):** locally verified, pending publication. Autonomous Research uses only the exact current robustness-build generation, requires both approved strategy versions with complete feature/market lineage, and fails closed without stale fallback. Regression passed 465 API, 32 Sentinel and 114 frontend tests plus all applicable local gates. This remains BB-092.
