...
**Status (2026-08-22): IMPLEMENTED AND LOCALLY VERIFIED; publication/CI pending.** A versioned snapshot-based operational gate now precedes scheduled BB-092 work. It reuses trusted CPU, memory and configured-disk metrics, fails closed on unavailable critical evidence and persists compact decisions with deterministic `Block > Defer > Allow` precedence. Local verification passed 486 API, 32 Sentinel and 115 frontend tests plus Release, frontend, API-image, documentation, Compose, diff and sanitized secret-pattern gates. Temperature/service activity, adaptive budgets, Hard Risk changes and all trading authority are excluded. The scheduler remains disabled by default. The next planned slice after acceptance is Finance Autonomous Research Operations / Recovery Hardening.
