...
**Status (2026-08-22): IMPLEMENTED AND LOCALLY VERIFIED; publication/CI pending.** A derived operational lifecycle, deterministic startup reconciliation, deduplicated true-operational incident streaks, stale/persistent-wait visibility, explicit maintenance pause, read-only operations API/UI and lawful compact-metadata backup are implemented. Local verification passed 495 API, 32 Sentinel and 115 frontend tests plus Release, frontend, API-image, documentation, Compose, diff and sanitized secret-pattern gates. Scientific rejection remains healthy research. Scheduler stays default-off and Finance remains `RESEARCH / 0 SEK / NONE` with no execution authority.
