...
**Status:** IN PROGRESS (closure evidence 2026-08-16). The actual BB-090 implementation commit is `6f6cc743b91b9d0c2cb52fb297d0215fd7125b50`; `fb55537aa77dd54c7a5929e3f29084bb5f86252e` published the initial status report. Closure code is deployed locally: adjusted-history audit, ordered migrations, full pre-migration backup, isolated drill, two restart checks, provider/strategy-neutral risk policy, exact deterministic UI risk aggregation and FRED quarantine promotion passed. Production retains 25 market revisions, 9,746 observations, 48 shadow predictions (24 valid PENDING plus 24 INVALIDATED audit rows), 0 outcomes and 0 risk evaluations. One revised-history FRED revision contains 58,196 observations and remains `REVISED_HISTORY_EXPLORATORY`. Closure remains blocked only on the owner configuring `FINANCE__FRED__APIKEY` with `FINANCE__FRED__ENABLED=true`, the bounded authoritative CPIAUCSL/UNRATE vintage drill, and final published GitHub Actions verification. No key value belongs in source, logs, SQLite or chat.
