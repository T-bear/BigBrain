...
**Status (2026-08-22): IMPLEMENTED, LOCAL VERIFICATION IN PROGRESS.** One semantic token/component system now serves Obsidian Gold, Arctic Wind and Forest Night. Obsidian Gold is the default; explicit selection persists through the existing Settings contract with local cache/offline fallback. A reusable mobile-first AppShell provides Hem, Familj, Media, Finance and Mer as primary destinations, with AI/Admin secondary. Existing module actions and Finance research/safety evidence remain present; technical Finance evidence uses progressive disclosure. The supplied local mockups were visual references and are intentionally not publication artifacts. Final full regression, publication and CI evidence remain required before COMPLETE.
