...
- Implementerat och verifierat: intern-only Compose-service, dedikerad qBittorrent-kategori/path, provider-neutral adapter, opaque kandidater, statusmappning och säker `CanCancel=false`. Commissioning verifierade healthy anslutning till Prowlarr/qBittorrent/Audiobookshelf, men upstream ignorerar Prowlarr-only-intentionen genom att ovillkorligt registrera built-in audiobook sources. Librarr är därför stoppad och ett nytt ägarbeslut krävs för en separat minimal source-registration-patch. Ingen acquisition skapades.
